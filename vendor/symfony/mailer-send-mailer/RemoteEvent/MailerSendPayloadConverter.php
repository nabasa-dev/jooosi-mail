 ...
 */
namespace JooosiMailDeps\Symfony\Component\Mailer\Bridge\MailerSend\RemoteEvent;
...
final class MailerSendPayloadConverter implements PayloadConverterInterface
{
    ...
    {
        if (\in_array($payload['type'], ['activity.sent', 'activity.delivered', 'activity.soft_bounced', 'activity.hard_bounced'], \true)) {
    ...
    private function getMetadata(array $payload): array
    {
        ...
        return match ($morphObject) {
            'open' => ['ip' => $payload['data']['morph']['ip'] ?? null],
            'click' => ['ip' => $payload['data']['morph']['ip'] ?? null, 'url' => $payload['data']['morph']['url'] ?? null],
            'recipient_unsubscribe' => ['reason' => $payload['data']['morph']['reason'] ?? null, 'readable_reason' => $payload['data']['morph']['readable_reason'] ?? null],
            default => [],
        };
    }
}
