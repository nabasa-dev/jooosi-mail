 ...
 */
namespace OmniMailDeps\Symfony\Component\Mailer\Bridge\MailerSend\Transport;
...
final class MailerSendApiTransport extends AbstractApiTransport
{
    ...
    {
        $response = $this->client->request('POST', 'https://' . $this->getEndpoint() . '/v1/email', ['json' => $this->getPayload($email, $envelope), 'headers' => ['Authorization' => 'Bearer ' . $this->key]]);
        try {
    ...
    private function getPayload(Email $email, Envelope $envelope): array
    {
        ...
        $payload = ['from' => array_filter(['email' => $sender->getAddress(), 'name' => $sender->getName()]), 'to' => $this->prepareAddresses($this->getRecipients($email, $envelope)), 'subject' => $email->getSubject()];
    ...
    private function prepareAddresses(array $addresses): array
    {
        ...
        foreach ($addresses as $address) {
            $recipients[] = ['email' => $address->getAddress(), 'name' => $address->getName()];
        }
        return $recipients;
    }
    private function prepareAttachments(Email $email): array
    {
        ...
        foreach ($email->getAttachments() as $attachment) {
            ...
            $att = ['content' => base64_encode($attachment->getBody()), 'filename' => $filename];
