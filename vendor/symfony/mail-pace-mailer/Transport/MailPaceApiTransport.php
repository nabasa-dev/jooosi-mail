 ...
 */
namespace JooosiMailDeps\Symfony\Component\Mailer\Bridge\MailPace\Transport;
...
final class MailPaceApiTransport extends AbstractApiTransport
{
    ...
    {
        $response = $this->client->request('POST', 'https://' . $this->getEndpoint() . '/send', ['headers' => ['Accept' => 'application/json', 'MailPace-Server-Token' => $this->key, 'Content-Type' => 'application/json', 'User-Agent' => 'MailPace Symfony Mailer'], 'json' => $this->getPayload($email, $envelope)]);
        try {
    ...
    private function getAttachments(Email $email): array
    {
        ...
        foreach ($email->getAttachments() as $attachment) {
            ...
            $att = ['name' => $filename, 'content' => $attachment->bodyToString(), 'content_type' => $headers->get('Content-Type')->getBody()];
