...
final class MailtrapApiTransport extends AbstractApiTransport
{
    ...
    {
        $response = $this->client->request('POST', 'https://' . $this->getEndpoint() . '/api/send' . (null !== $this->inboxId ? '/' . $this->inboxId : ''), ['json' => $this->getPayload($email, $envelope), 'auth_bearer' => $this->token]);
        try {
    ...
    {
        $payload = ['from' => self::encodeEmail($envelope->getSender()), 'to' => array_map(self::encodeEmail(...), $email->getTo()), 'cc' => array_map(self::encodeEmail(...), $email->getCc()), 'bcc' => array_map(self::encodeEmail(...), $email->getBcc()), 'subject' => $email->getSubject(), 'text' => $email->getTextBody(), 'html' => $email->getHtmlBody(), 'attachments' => $this->getAttachments($email)];
        foreach ($email->getHeaders()->all() as $name => $header) {
            if (\in_array($name, self::HEADERS_TO_BYPASS, \true)) {
                continue;
            }
    ...
    private function getAttachments(Email $email): array
    {
        ...
        foreach ($email->getAttachments() as $attachment) {
            ...
            $att = ['content' => $attachment->bodyToString(), 'type' => $headers->get('Content-Type')->getBody(), 'filename' => $filename, 'disposition' => $disposition];
