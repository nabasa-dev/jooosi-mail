 ...
 */
namespace JooosiMailDeps\Symfony\Component\Mailer\Bridge\Infobip\Transport;
...
final class InfobipApiTransport extends AbstractApiTransport
{
    ...
    protected function doSendApi(SentMessage $sentMessage, Email $email, Envelope $envelope): ResponseInterface
    {
        ...
        $response = $this->client->request('POST', \sprintf('https://%s/email/%s/send', $this->getEndpoint(), self::API_VERSION), ['headers' => $headers, 'body' => $formData->bodyToIterable()]);
        try {
    ...
    {
        $fields = ['from' => $envelope->getSender()->toString(), 'subject' => $email->getSubject()];
