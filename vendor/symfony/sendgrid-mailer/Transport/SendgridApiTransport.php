 ...
 */
namespace OmniMailDeps\Symfony\Component\Mailer\Bridge\Sendgrid\Transport;

use OmniMailDeps\Psr\EventDispatcher\EventDispatcherInterface;
use OmniMailDeps\Psr\Log\LoggerInterface;
use OmniMailDeps\Symfony\Component\Mailer\Bridge\Sendgrid\Header\SuppressionGroupHeader;
...
use OmniMailDeps\Symfony\Component\Mime\Header\DateHeader;
...
class SendgridApiTransport extends AbstractApiTransport
{
    ...
    {
        $response = $this->client->request('POST', 'https://' . $this->getEndpoint() . '/v3/mail/send', ['json' => $this->getPayload($email, $envelope), 'auth_bearer' => $this->key]);
        try {
    ...
    private function getPayload(Email $email, Envelope $envelope): array
    {
        ...
        $payload = ['personalizations' => [], 'from' => $addressStringifier($envelope->getSender()), 'content' => $this->getContent($email)];
        if ($email->getAttachments()) {
            $payload['attachments'] = $this->getAttachments($email);
        }
        $personalization = ['to' => array_map($addressStringifier, $this->getRecipients($email, $envelope)), 'subject' => $email->getSubject()];
        ...
        foreach ($email->getHeaders()->all() as $name => $header) {
            ...
            if (\in_array($name, ['x-sg-id', 'x-sg-eid', 'received', 'dkim-signature', 'content-transfer-encoding', 'from', 'to', 'cc', 'bcc', 'subject', 'content-type', 'reply-to'], \true)) {
                continue;
            }
            if ('send-at' === $name) {
    ...
    private function getAttachments(Email $email): array
    {
        ...
        foreach ($email->getAttachments() as $attachment) {
            ...
            $att = ['content' => str_replace("\r\n", '', $attachment->bodyToString()), 'type' => $headers->get('Content-Type')->getBody(), 'filename' => $filename, 'disposition' => $disposition];
