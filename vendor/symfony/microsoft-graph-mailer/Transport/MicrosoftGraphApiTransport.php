...
class MicrosoftGraphApiTransport extends AbstractApiTransport
{
     ...
     */
    public function __construct(private readonly string $graphEndpoint, private readonly TokenManager $tokenManager, private readonly bool $noSave, ?HttpClientInterface $client = null, ?EventDispatcherInterface $dispatcher = null, ?LoggerInterface $logger = null)
    {
    ...
    protected function doSendApi(SentMessage $sentMessage, Email $email, Envelope $envelope): ResponseInterface
    {
        ...
        $response = $this->client->request('POST', $endpoint, ['json' => $payload, 'auth_bearer' => $this->tokenManager->getToken()]);
        try {
    ...
    {
        $message = ['sender' => $this->getEmailAddress($envelope->getSender()), 'subject' => $email->getSubject(), 'body' => $this->getBodyPayload($email), 'importance' => $this->getImportanceLevel($email), 'toRecipients' => array_map($this->getEmailAddress(...), $email->getTo())];
        if ($email->getFrom()) {
    ...
    private function getBodyPayload(Email $email): array
    {
        ...
        return ['content' => $email->getTextBody(), 'contentType' => 'text'];
    }
    ...
    private function getMessageAttachments(Email $email): array
    {
        ...
        foreach ($email->getAttachments() as $attachment) {
            ...
            $attr = ['@odata.type' => '#microsoft.graph.fileAttachment', 'name' => $filename, 'contentBytes' => base64_encode($attachment->getBody()), 'contentType' => $headers->get('Content-Type')->getBody()];
    ...
    private function getImportanceLevel(Email $email): string
    {
        return match ($email->getPriority()) {
            Email::PRIORITY_HIGHEST, Email::PRIORITY_HIGH => 'high',
            Email::PRIORITY_LOW, Email::PRIORITY_LOWEST => 'low',
            default => 'normal',
        };
    }
}
