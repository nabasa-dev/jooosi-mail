 ...
 */
namespace OmniMailDeps\Symfony\Component\Mailer\Bridge\Azure\Transport;
...
final class AzureApiTransport extends AbstractApiTransport
{
    ...
    protected function doSendApi(SentMessage $sentMessage, Email $email, Envelope $envelope): ResponseInterface
    {
        ...
        $response = $this->client->request('POST', 'https://' . $endpoint, ['body' => json_encode($payload), 'headers' => $this->getSignedHeaders($payload)]);
        try {
        ...
        $sentMessage->setMessageId(json_decode($response->getContent(\false), \true)['id']);
        return $response;
    }
    /**
    ...
    private function getPayload(Email $email, Envelope $envelope): array
    {
        ...
        $data = ['content' => ['html' => $email->getHtmlBody(), 'plainText' => $email->getTextBody(), 'subject' => $email->getSubject()], 'recipients' => ['to' => array_map($addressStringifier, $this->getRecipients($email, $envelope))], 'senderAddress' => $envelope->getSender()->getAddress(), 'attachments' => $this->getMessageAttachments($email), 'userEngagementTrackingDisabled' => $this->disableTracking, 'headers' => ($headers = $this->getMessageCustomHeaders($email)) ? $headers : null, 'importance' => $this->getPriorityLevel($email->getPriority())];
    ...
    private function getMessageAttachments(Email $email): array
    {
        ...
        foreach ($email->getAttachments() as $attachment) {
            ...
            $att = ['name' => $filename, 'contentInBase64' => base64_encode($attachment->getBody()), 'contentType' => $headers->get('Content-Type')->getBody()];
    ...
    {
        return base64_encode(hash('sha256', $content, \true));
    }
    /**
    ...
    private function generateAuthenticationSignature(string $content): string
    {
        ...
        $hashedBytes = hash_hmac('sha256', mb_convert_encoding($content, 'UTF-8'), $key, \true);
    ...
    private function getSignedHeaders(array $payload): array
    {
        ...
        return ['Content-Type' => 'application/json', 'repeatability-request-id' => $messageId, 'Operation-Id' => $messageId, 'repeatability-first-sent' => $utcNow, 'x-ms-date' => $utcNow, 'x-ms-content-sha256' => $contentHash, 'x-ms-client-request-id' => $messageId, 'Authorization' => "HMAC-SHA256 SignedHeaders=x-ms-date;host;x-ms-content-sha256&Signature={$signature}"];
    }
    /**
