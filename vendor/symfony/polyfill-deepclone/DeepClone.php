...
final class DeepClone
{
    ...
    private const NOT_ROUND_TRIPPABLE = ['ZipArchive' => \true, 'XMLWriter' => \true, 'XMLReader' => \true, 'SNMP' => \true, 'tidy' => \true];
    ...
    private static array $propertyScopes = [];
    // [class][key] = [declaring class, real name]; key is bare name, "\0*\0name", or "\0class\0name"
    ...
    public static function deepclone_to_array(mixed $value, ?array $allowed_classes = null): array
    {
        ...
        foreach ($resolve as $scope => $names) {
            foreach ($names as $name => $ids) {
                foreach ($ids as $id => $marker) {
                    ...
                    if (!$v instanceof \Symfony\Polyfill\DeepClone\Reference) {
                        $m = null;
        ...
        $data = ['classes' => 1 === \count($classes) ? $classes[0] : ($classes ?: ''), 'objectMeta' => $n, 'prepared' => $prepared];
    ...
    public static function deepclone_from_array(array $data, ?array $allowed_classes = null): mixed
    {
        ...
        return self::reconstruct($data['prepared'], $objectMeta, $numObjects, $data['properties'] ?? [], $data['resolve'] ?? [], $data['states'] ?? [], $data['refs'] ?? [], $data['mask'] ?? null, $data['refMasks'] ?? [], $allowed_classes, $expectedStates);
    }
    public static function deepclone_hydrate(object|string $object_or_class, array $vars = [], int $flags = 0): object
    {
        ...
        foreach ($vars as $name => $_) {
            if (\array_key_exists($name, $propertyScopes) ? $class !== $propertyScopes[$name][0] : "\x00" === ($name[0] ?? '')) {
                $needsGroup = \true;
                break;
            }
        }
        ...
        foreach ($vars as $name => &$value) {
            ...
            $sep = strpos($name, "\x00", 1);
            if (\false === $sep || 1 === $sep || str_contains(substr($name, $sep + 1), "\x00")) {
            ...
            if ('*' !== $scopeName && 'stdClass' !== $scopeName && $scopeName !== $class && (!is_a($class, $scopeName, \true) || interface_exists($scopeName, \false))) {
    ...
    private static function getPropertyScopes(\ReflectionClass $class): array
    {
        ...
        foreach ($class->getProperties() as $property) {
            ...
            if ($property->isPrivate()) {
                $propertyScopes["\x00" . $property->class . "\x00" . $name] = $propertyScopes[$name] = $entry;
                continue;
            }
            $propertyScopes[$name] = $entry;
            if ($property->isProtected()) {
                $propertyScopes["\x00*\x00" . $name] = $entry;
            }
        }
        while ($class = $class->getParentClass()) {
            foreach ($class->getProperties(\ReflectionProperty::IS_PRIVATE) as $property) {
                ...
                $propertyScopes["\x00" . $property->class . "\x00" . $property->name] = $entry;
    ...
    private static function prepare($values, &$objectsPool, &$refsPool, &$objectsCount, &$valuesAreStatic, &$mask = null, ?array $allowedSet = null)
    {
        ...
        foreach ($values as $k => $value) {
            ...
            if ($isRef = !$valueIsStatic = $values[$k] !== $sentinel) {
                $values[$k] =& $value;
                // Break hard reference
                unset($value);
                $refs[$k] = $value = $values[$k];
                if ($value instanceof \Symfony\Polyfill\DeepClone\Reference && 0 > $value->id) {
                ...
                $refs[$k] = $values[$k] = new \Symfony\Polyfill\DeepClone\Reference(-\count($refsPool), $value);
            }
            if (\is_array($value)) {
                if ($value) {
                    $m = null;
            ...
            if ($value instanceof \Closure && ($r = new \ReflectionFunction($value)) && !(\PHP_VERSION_ID >= 80200 ? $r->isAnonymous() : str_contains($r->name, "@anonymous\x00"))) {
                ...
                $value = !($rm?->isPublic() ?? \true) ? [$callable, $rm->class, $rm->name] : $callable;
                $mask[$k] = 0;
                goto handle_value;
            }
            $class = $value::class;
    ...
    private static function reconstruct($prepared, $objectMeta, $numObjects, $properties, $resolve, $states, $refs, $preparedMask = null, $refMasks = [], ?array $allowedClasses = null, array $expectedStates = [])
    {
        ...
        foreach ($objectMeta as $id => [$class]) {
            ...
            if (self::$needsFullUnserialize[$class] ?? \false) {
                $objects[$id] = null;
                // placeholder — finalized below or in states loop
    ...
    private static function replaceRefs($value, &$mask)
    {
        if (\is_array($value)) {
            foreach ($value as $k => $v) {
                if ($v instanceof \Symfony\Polyfill\DeepClone\Reference || \is_array($v)) {
                    $m = null;
        ...
        if (!$value instanceof \Symfony\Polyfill\DeepClone\Reference) {
            return $value;
        }
        if ($value->id >= 0) {
            $mask = \true;
    ...
    private static function getSimpleHydrator(string $class, int $flags = 0): \Closure
    {
        ...
        foreach ($classReflector->getProperties() as $propertyReflector) {
            ...
            if (\PHP_VERSION_ID >= 80400 && !$propertyReflector->isAbstract() && $propertyReflector->getHooks()) {
                ...
                $unsetOnNull[$propertyReflector->name] = \true;
            }
            // Property-type-only decision: hook presence and CALL_HOOKS don't influence it.
            if (($t ??= $propertyReflector->getType()) instanceof \ReflectionNamedType && !$t->isBuiltin() && enum_exists($enumName = $t->getName()) && (new \ReflectionEnum($enumName))->getBackingType()) {
