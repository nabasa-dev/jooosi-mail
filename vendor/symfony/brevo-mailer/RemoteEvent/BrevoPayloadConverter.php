 ...
 */
namespace JooosiMailDeps\Symfony\Component\Mailer\Bridge\Brevo\RemoteEvent;
...
final class BrevoPayloadConverter implements PayloadConverterInterface
{
    ...
    {
        if (\in_array($payload['event'], ['request', 'deferred', 'delivered', 'soft_bounce', 'hard_bounce', 'invalid_email', 'blocked', 'error'], \true)) {
        ...
        if (\in_array($payload['event'], ['deferred', 'soft_bounce', 'hard_bounce', 'invalid_email', 'blocked', 'error'], \true) && isset($payload['reason'])) {
