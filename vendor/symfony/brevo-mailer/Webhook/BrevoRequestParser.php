...
final class BrevoRequestParser extends AbstractRequestParser
{
    ...
    public function __construct(private readonly BrevoPayloadConverter $converter, private readonly array $allowedIPs = self::PROVIDER_IPS)
    {
    }
    protected function getRequestMatcher(): RequestMatcherInterface
    {
        return new ChainRequestMatcher([new MethodRequestMatcher('POST'), new IsJsonRequestMatcher(), new IpsRequestMatcher($this->allowedIPs)]);
    }
    ...
    ): ?AbstractMailerEvent
    {
        ...
        if (!isset($content['event']) || !isset($content['email']) || !isset($content['message-id']) || !isset($content['ts_event'])) {
