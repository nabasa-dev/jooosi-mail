 ...
 */
namespace JooosiMailDeps\Symfony\Component\Mailer\Bridge\Brevo\Transport;
...
final class BrevoApiTransport extends AbstractApiTransport
{
    ...
    {
        $response = $this->client->request('POST', 'https://' . $this->getEndpoint() . '/v3/smtp/email', ['json' => $this->getPayload($email, $envelope), 'headers' => ['api-key' => $this->key]]);
        try {
    ...
    private function prepareAttachments(Email $email): array
    {
        ...
        foreach ($email->getAttachments() as $attachment) {
            ...
            $att = ['content' => str_replace("\r\n", '', $attachment->bodyToString()), 'name' => $filename];
            $attachments[] = $att;
        }
        return $attachments;
    }
