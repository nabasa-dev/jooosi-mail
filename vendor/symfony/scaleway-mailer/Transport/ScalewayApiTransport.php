...
final class ScalewayApiTransport extends AbstractApiTransport
{
    ...
    protected function doSendApi(SentMessage $sentMessage, Email $email, Envelope $envelope): ResponseInterface
    {
        ...
        $response = $this->client->request('POST', 'https://' . $this->getEndpoint() . $path, ['json' => $this->getPayload($email, $envelope), 'headers' => ['X-Auth-Token' => $this->token]]);
        try {
    ...
    {
        $payload = ['from' => $this->formatAddress($envelope->getSender()), 'to' => $this->formatAddresses($this->getRecipients($email, $envelope)), 'subject' => $email->getSubject(), 'project_id' => $this->projectId];
    ...
    private function prepareAttachments(Email $email): array
    {
        ...
        foreach ($email->getAttachments() as $attachment) {
            ...
            $attachments[] = ['name' => $filename, 'type' => $headers->get('Content-Type')->getBody(), 'content' => str_replace("\r\n", '', $attachment->bodyToString())];
        }
        return $attachments;
    }
