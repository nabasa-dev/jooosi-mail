 ...
 */
namespace JooosiMailDeps\Symfony\Component\Mailer\Bridge\Mailjet\Transport;
...
class MailjetApiTransport extends AbstractApiTransport
{
    ...
    {
        $response = $this->client->request('POST', \sprintf('https://%s/v%s/send', $this->getEndpoint(), self::API_VERSION), ['headers' => ['Accept' => 'application/json'], 'auth_basic' => $this->publicKey . ':' . $this->privateKey, 'json' => $this->getPayload($email, $envelope)]);
        try {
    ...
    private function getPayload(Email $email, Envelope $envelope): array
    {
        ...
        $message = ['From' => $this->formatAddress($envelope->getSender()), 'To' => $this->formatAddresses($this->getRecipients($email, $envelope)), 'Subject' => $email->getSubject(), 'Attachments' => $attachments, 'InlinedAttachments' => $inlines];
        ...
        foreach ($email->getHeaders()->all() as $headerName => $header) {
            ...
            if (\in_array($headerName, self::FORBIDDEN_HEADERS, \true)) {
                continue;
            }
            $message['Headers'][$header->getName()] = $header->getBodyAsString();
        }
        return ['Messages' => [$message], 'SandBoxMode' => $this->sandbox];
    }
    ...
    private function prepareAttachments(Email $email, ?string $html): array
    {
        ...
        foreach ($email->getAttachments() as $attachment) {
            ...
            $formattedAttachment = ['ContentType' => $attachment->getMediaType() . '/' . $attachment->getMediaSubtype(), 'Filename' => $filename, 'Base64Content' => $attachment->bodyToString()];
    ...
    private function castCustomHeader(string $value, string $type): mixed
    {
        return match ($type) {
            ...
            'int' => (int) $value,
            'json' => json_decode($value, \true, 512, \JSON_THROW_ON_ERROR),
            'string' => $value,
