...
use JooosiMailDeps\Symfony\Component\Mailer\Bridge\AhaSend\Event\AhaSendDeliveryEvent;
...
final class AhaSendApiTransport extends AbstractApiTransport
{
    ...
    {
        $response = $this->client->request('POST', 'https://' . $this->getEndpoint() . '/v1/email/send', ['json' => $this->getPayload($email, $envelope), 'headers' => ['X-Api-Key' => $this->apiKey]]);
        try {
    ...
    private function getPayload(Email $email, Envelope $envelope): array
    {
        ...
        $payload = ['recipients' => $this->formatAddresses($envelope->getRecipients()), 'from' => $this->formatAddress($envelope->getSender()), 'content' => ['subject' => $email->getSubject()]];
    ...
    private function prepareHeaders(Headers $headers): array
    {
        ...
        foreach ($headers->all() as $header) {
            if (\in_array($header->getName(), $headersToBypass, \true)) {
                continue;
            }
    ...
    private function getAttachments(Email $email): array
    {
        ...
        foreach ($email->getAttachments() as $attachment) {
            ...
            $att = ['content_type' => $headers->get('Content-Type')->getBody(), 'file_name' => $attachment->getFilename(), 'data' => $body, 'base64' => $base64];
