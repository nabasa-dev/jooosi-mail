        ...
        foreach ($container->findTaggedResourceIds('container.tag_decorator', \false) as $id => $tags) {
            ...
            foreach ($tags as $tag) {
                ...
                foreach ($taggedServices as $taggedServiceId => $_) {
                    $container->setDefinition(\sprintf('.decorator.%s.%s', $taggedServiceId, $id), $definitionCloner->clone())->clearTag('container.tag_decorator')->clearTag('container.excluded')->setDecoratedService($taggedServiceId, null, $priority, $invalidBehavior);
                }
            }
