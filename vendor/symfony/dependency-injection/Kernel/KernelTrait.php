...
trait KernelTrait
{
    ...
    protected function initializeBundles(): void
    {
        ...
        if (is_file($cachePath) && (!$this->debug || is_file($bundlesPath = $this->getBundlesPath()) && filemtime($cachePath) > filemtime($bundlesPath))) {
    ...
    protected function dumpContainer(ConfigCache $cache, ContainerBuilder $container, string $class, string $baseClass): void
    {
        ...
        foreach ($this->bundleClasses as $name => $bundleClass) {
            $code .= \sprintf("    %s => new \\%s(),\n", var_export($name, \true), $bundleClass);
        }
        $code .= "];\n";
    ...
    protected function getContainerLoader(ContainerInterface $container): DelegatingLoader
    {
        ...
        $resolver = new LoaderResolver([new YamlFileLoader($container, $locator, $env), new IniFileLoader($container, $locator, $env), new PhpFileLoader($container, $locator, $env), new GlobFileLoader($container, $locator, $env), new DirectoryLoader($container, $locator, $env), new ClosureLoader($container, $env)]);
    ...
    private function registerContainerConfiguration(LoaderInterface $loader): void
    {
        $loader->load(function (ContainerBuilder $container) use ($loader) {
            $kernelClass = str_contains(static::class, "@anonymous\x00") ? get_parent_class(static::class) : static::class;
            if (!$container->hasDefinition('kernel')) {
                $container->register('kernel', $kernelClass)->setSynthetic(\true)->setPublic(\true);
            }
    ...
    private function getKernelParameters(): array
    {
        ...
        foreach ($this->bundles as $name => $bundle) {
            ...
            $bundlesMetadata[$name] = ['path' => $bundle->getPath()];
        }
        ...
        return ['kernel.project_dir' => realpath($this->getProjectDir()) ?: $this->getProjectDir(), 'kernel.environment' => $this->environment, 'kernel.runtime_environment' => '%env(default:kernel.environment:APP_RUNTIME_ENV)%', 'kernel.runtime_mode' => '%env(query_string:default:container.runtime_mode:APP_RUNTIME_MODE)%', 'kernel.runtime_mode.web' => '%env(bool:default::key:web:default:kernel.runtime_mode:)%', 'kernel.runtime_mode.cli' => '%env(not:default:kernel.runtime_mode.web:)%', 'kernel.runtime_mode.worker' => '%env(int:default::key:worker:default:kernel.runtime_mode:)%', 'kernel.debug' => $this->debug, 'kernel.build_dir' => realpath($dir = $this->getEffectiveBuildDir()) ?: $dir, 'kernel.cache_dir' => realpath($dir = $this->getCacheDir() === $this->getBuildDir() ? $this->getEffectiveBuildDir() : $this->getCacheDir()) ?: $dir, 'kernel.bundles' => $bundles, 'kernel.bundles_metadata' => $bundlesMetadata, 'kernel.container_class' => $this->getContainerClass(), '.kernel.config_dir' => $this->getConfigDir(), '.kernel.bundles_definition' => $this->getBundlesDefinition(), '.container.known_envs' => array_keys($knownEnvs)] + (null !== ($dir = $this->getLogDir()) ? ['kernel.logs_dir' => realpath($dir) ?: $dir] : []) + (null !== ($dir = $this->getShareDir()) ? ['kernel.share_dir' => realpath($dir) ?: $dir] : []);
    }
