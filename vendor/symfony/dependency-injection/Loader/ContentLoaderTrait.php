...
trait ContentLoaderTrait
{
    ...
    private function parseDefinition(string $id, array|string|null $service, string $file, array $defaults, bool $return = \false, bool $trackBindings = \true): Definition|Alias|null
    {
        ...
        if (isset($service['stack'])) {
            ...
            $service = ['parent' => '', 'arguments' => $stack, 'tags' => ['container.stack'], 'public' => $service['public'] ?? null, 'deprecated' => $service['deprecated'] ?? null, 'decorates' => $service['decorates'] ?? null, 'decorates_tag' => $service['decorates_tag'] ?? null, 'decoration_inner_name' => $service['decoration_inner_name'] ?? null, 'decoration_priority' => $service['decoration_priority'] ?? null, 'decoration_on_invalid' => $service['decoration_on_invalid'] ?? null];
        }
        ...
        if (null !== $decoratesTag = $service['decorates_tag'] ?? null) {
            ...
            $tagAttributes = ['decorates_tag' => $decoratesTag, 'priority' => $priority];
        ...
        if ($value instanceof TaggedValue) {
            ...
            if ('env_closure' === $value->getTag()) {
                if (\is_array($argument)) {
                    ...
                    $stringable = $argument[2] ?? \true;
                    $validKeys = !array_diff(array_keys($argument), [0, 1, 2]);
                } else {
                    [$envExpr, $default, $stringable, $validKeys] = [$argument, null, \false, \true];
                }
            ...
            if (\in_array($value->getTag(), ['tagged_iterator', 'tagged_locator'], \true)) {
                ...
                if (\is_array($argument) && isset($argument['tag']) && $argument['tag']) {
                    ...
                    $excludeSelf = $argument['exclude_self'] ?? \true;
