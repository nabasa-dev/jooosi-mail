...
class CachingHttpClient implements HttpClientInterface, LoggerAwareInterface, ResetInterface
{
     ...
     */
    private const EXCLUDED_HEADERS = ['connection' => \true, 'keep-alive' => \true, 'proxy-authenticate' => \true, 'proxy-authentication-info' => \true, 'proxy-authorization' => \true, 'te' => \true, 'trailer' => \true, 'transfer-encoding' => \true, 'upgrade' => \true];
    /**
     ...
     *                                  to this maximum.
     */
    public function __construct(private HttpClientInterface $client, private readonly TagAwareCacheInterface $cache, array $defaultOptions = [], private readonly bool $sharedCache = \true, private readonly ?int $maxTtl = 86400)
    {
        if (null === $maxTtl) {
    ...
    public function request(string $method, string $url, array $options = []): ResponseInterface
    {
        ...
        if ('' !== $options['body'] || ($options['extra']['no_cache'] ?? \false) || isset($options['normalized_headers']['range']) || !\in_array($method, self::CACHEABLE_METHODS, \true)) {
            ...
            $passthru = function (ChunkInterface $chunk, AsyncContext $context) use ($method, $fullUrlTag): \Generator {
                ...
                if ($statusCode >= 200 && $statusCode < 400 && !\in_array($method, self::SAFE_METHODS, \true)) {
                    $this->cache->invalidateTags([$fullUrlTag]);
                }
                $context->passthru();
                yield $chunk;
            };
            $this->isInnerRequest = \true;
            try {
        ...
        $passthru = function (ChunkInterface $chunk, AsyncContext $context) use ($expiresAt, $fullUrlTag, $requestHash, $varyKey, $varyFields, &$metadataKey, $cachedData, $freshness, $url, $method, $options, $hasClientConditionalValidator, $sentCacheValidator, $allowStaleFallback): \Generator {
            ...
            if (null !== $chunk->getError() || $chunk->isTimeout()) {
                ...
                if ($allowStaleFallback && Freshness::StaleButUsable === $freshness) {
                    $this->logger?->info('Serving stale cached response for "{method} {url}" because the upstream call failed: {error}.', ['method' => $method, 'url' => $url, 'error' => $chunk instanceof ErrorChunk ? $chunk->getError() : 'timeout']);
            ...
            if ($chunk->isFirst()) {
                ...
                if (304 === $statusCode && null !== $freshness) {
                    ...
                    $updatedMetadataIsCacheable = !\in_array('*', $newVaryFields, \true) && $varyFields === $newVaryFields && $this->isServerResponseCacheable($cachedData['status_code'], $options['normalized_headers'], $updatedCachedData['headers'], $updatedCacheControl);
                ...
                if ($statusCode >= 500 && $statusCode < 600) {
                    if ($allowStaleFallback && Freshness::StaleButUsable === $freshness) {
                        $this->logger?->info('Serving stale cached response for "{method} {url}" because the upstream returned a server error (HTTP {status}).', ['method' => $method, 'url' => $url, 'status' => $statusCode]);
                        $context->passthru();
                        $context->replaceResponse($this->createResponseFromCache($cachedData, $method, $url, $options, $metadataKey));
                        return;
                    }
    ...
    private static function getExcludedHeaders(array $headers): array
    {
        ...
        foreach ($headers['connection'] ?? [] as $connectionHeader) {
            foreach (explode(',', $connectionHeader) as $headerName) {
                ...
                if ('' !== $headerName) {
                    $excludedHeaders[$headerName] = \true;
                }
            }
        }
    ...
    private static function notModifiedResponseMatchesCachedResponse(array $headers, array $cachedData): bool
    {
        ...
        foreach ($headers['last-modified'] ?? [] as $lastModified) {
            return \in_array($lastModified, $cachedData['headers']['last-modified'] ?? [], \true);
        }
    ...
    private function isServerResponseCacheable(int $statusCode, array $requestHeaders, array $responseHeaders, array $cacheControl): bool
    {
        ...
        if (\in_array($statusCode, self::DEFAULT_CACHEABLE_STATUS_CODES, \true)) {
            return \true;
        }
