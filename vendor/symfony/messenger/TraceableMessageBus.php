...
class TraceableMessageBus implements MessageBusInterface
{
    ...
    private function getCaller(): array
    {
        ...
        for ($i = 2; $i < 8 && !$found; ++$i) {
            if (isset($trace[$i]['class'], $trace[$i]['function']) && 'dispatch' === $trace[$i]['function'] && is_a($trace[$i]['class'], MessageBusInterface::class, \true)) {
                $file = $trace[$i]['file'];
                $line = $trace[$i]['line'];
                while (++$i < 8) {
        ...
        return ['name' => substr($name, strrpos($name, '/') + 1), 'file' => $file, 'line' => $line];
    }
}
