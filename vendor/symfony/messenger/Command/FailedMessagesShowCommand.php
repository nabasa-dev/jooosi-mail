...
class FailedMessagesShowCommand extends AbstractFailedMessagesCommand
{
    ...
    {
        $this->setDefinition([new InputArgument('id', InputArgument::OPTIONAL, 'Specific message id to show'), new InputOption('max', null, InputOption::VALUE_REQUIRED, 'Maximum number of messages to list', 50), new InputOption('transport', null, InputOption::VALUE_REQUIRED, 'Use a specific failure transport', self::DEFAULT_TRANSPORT_OPTION), new InputOption('stats', null, InputOption::VALUE_NONE, 'Display the message count by class'), new InputOption('class-filter', null, InputOption::VALUE_REQUIRED, 'Filter by a specific class name')])->setHelp(<<<'EOF'
...
EOF
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($input->getOption('stats')) {
            $max = $input->hasParameterOption(['--max'], \true) ? $input->getOption('max') : null;
    ...
    private function listMessages(ListableReceiverInterface $receiver, string $failedTransportName, SymfonyStyle $io, SymfonyStyle $errorIo, int $max, ?string $classFilter = null): void
    {
        ...
        try {
            foreach ($envelopes as $envelope) {
                ...
                $rows[] = [$this->getMessageId($envelope), $currentClassName, null === $lastRedeliveryStamp ? '' : $lastRedeliveryStamp->getRedeliveredAt()->format('Y-m-d H:i:s'), $lastErrorDetailsStamp?->getExceptionMessage() ?? ''];
            }
        } finally {
    ...
    private function showMessage(ListableReceiverInterface $receiver, string $failedTransportName, string $id, SymfonyStyle $io, SymfonyStyle $errorIo): void
    {
        ...
        $io->writeln(['', \sprintf(' Run <comment>messenger:failed:retry %s --transport=%s</comment> to retry this message.', $id, $failedTransportName), \sprintf(' Run <comment>messenger:failed:remove %s --transport=%s</comment> to delete it.', $id, $failedTransportName)]);
    }
}
