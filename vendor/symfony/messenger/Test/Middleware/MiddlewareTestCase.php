...
{
    protected function getStackMock(bool $nextIsCalled = \true)
    {
        if (!$nextIsCalled) {
        ...
        $nextMiddleware->expects($this->once())->method('handle')->willReturnCallback(static fn(Envelope $envelope, StackInterface $stack): Envelope => $envelope);
        return new StackMiddleware($nextMiddleware);
    }
    protected function getThrowingStackMock(?\Throwable $throwable = null)
    {
        $nextMiddleware = $this->createMock(MiddlewareInterface::class);
        $nextMiddleware->expects($this->once())->method('handle')->willThrowException($throwable ?? new \RuntimeException('Thrown from next middleware.'));
        return new StackMiddleware($nextMiddleware);
    }
}
