 ...
 */
namespace JooosiMailDeps\Symfony\Component\Messenger\DataCollector;
...
class MessengerDataCollector extends DataCollector implements LateDataCollectorInterface
{
    ...
    private function collectMessage(string $busName, array $tracedMessage): array
    {
        ...
        $debugRepresentation = ['bus' => $busName, 'stamps' => $tracedMessage['stamps'] ?? null, 'stamps_after_dispatch' => $tracedMessage['stamps_after_dispatch'] ?? null, 'message' => ['type' => new ClassStub($message::class), 'value' => $message], 'caller' => $tracedMessage['caller']];
        if (isset($tracedMessage['exception'])) {
            $exception = $tracedMessage['exception'];
            $debugRepresentation['exception'] = ['type' => $exception::class, 'value' => $exception];
        }
