...
class HandlerFailedException extends RuntimeException implements WrappedExceptionsInterface, EnvelopeAwareExceptionInterface
{
     ...
     */
    public function __construct(private Envelope $envelope, array $exceptions)
    {
        $firstFailure = current($exceptions);
        $message = \sprintf('Handling "%s" failed: ', $envelope->getMessage()::class);
        parent::__construct($message . (1 === \count($exceptions) ? $firstFailure->getMessage() : \sprintf('%d handlers failed. First failure is: %s', \count($exceptions), $firstFailure->getMessage())), (int) $firstFailure->getCode(), $firstFailure);
