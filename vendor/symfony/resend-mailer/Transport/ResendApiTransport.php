...
final class ResendApiTransport extends AbstractApiTransport
{
    ...
    {
        $response = $this->client->request('POST', 'https://' . $this->getEndpoint() . '/emails', ['json' => $this->getPayload($email, $envelope), 'headers' => ['Authorization' => 'Bearer ' . $this->apiKey]]);
        try {
    ...
    private function prepareAttachments(Email $email): array
    {
        ...
        foreach ($email->getAttachments() as $attachment) {
            $attachments[] = ['filename' => $attachment->getPreparedHeaders()->getHeaderParameter('Content-Disposition', 'filename'), 'content' => str_replace("\r\n", '', $attachment->bodyToString())];
        }
        return $attachments;
    }
    private function prepareHeadersAndTags(Headers $headers): array
    {
        ...
        foreach ($headers->all() as $name => $header) {
            if (\in_array($name, ['from', 'to', 'cc', 'bcc', 'subject', 'reply_to'], \true)) {
                continue;
            }
