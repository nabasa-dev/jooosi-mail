...
class MandrillHttpTransport extends AbstractHttpTransport
{
    ...
    protected function doSendHttp(SentMessage $message): ResponseInterface
    {
        ...
        $response = $this->client->request('POST', 'https://' . $this->getEndpoint() . '/api/1.0/messages/send-raw.json', ['json' => ['key' => $this->key, 'to' => array_map(static fn(Address $recipient): string => $recipient->getAddress(), $envelope->getRecipients()), 'from_email' => $envelope->getSender()->getAddress(), 'from_name' => $envelope->getSender()->getName(), 'raw_message' => $message->toString()]]);
        try {
