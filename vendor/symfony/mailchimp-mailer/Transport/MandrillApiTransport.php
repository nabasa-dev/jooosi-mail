 ...
 */
namespace JooosiMailDeps\Symfony\Component\Mailer\Bridge\Mailchimp\Transport;
...
class MandrillApiTransport extends AbstractApiTransport
{
    ...
    {
        $response = $this->client->request('POST', 'https://' . $this->getEndpoint() . '/api/1.0/messages/send.json', ['json' => $this->getPayload($email, $envelope)]);
        try {
    ...
    private function getPayload(Email $email, Envelope $envelope): array
    {
        ...
        foreach ($email->getAttachments() as $attachment) {
            ...
            $att = ['content' => $attachment->bodyToString(), 'type' => $headers->get('Content-Type')->getBody()];
        ...
        foreach ($email->getHeaders()->all() as $name => $header) {
            ...
            if ($header instanceof TagHeader) {
                $payload['message']['tags'] = array_merge($payload['message']['tags'] ?? [], explode(',', $header->getValue()));
                continue;
            }
    ...
    private function getRecipientsPayload(Email $email, Envelope $envelope): array
    {
        ...
        foreach ($envelope->getRecipients() as $recipient) {
            $type = 'to';
            if (\in_array($recipient, $email->getBcc(), \true)) {
                $type = 'bcc';
            } elseif (\in_array($recipient, $email->getCc(), \true)) {
                $type = 'cc';
            }
            $recipientPayload = ['email' => $recipient->getAddress(), 'type' => $type];
