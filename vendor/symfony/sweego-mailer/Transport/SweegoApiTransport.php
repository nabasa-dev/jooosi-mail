 ...
 */
namespace JooosiMailDeps\Symfony\Component\Mailer\Bridge\Sweego\Transport;
...
final class SweegoApiTransport extends AbstractApiTransport
{
    ...
    {
        $response = $this->client->request('POST', 'https://' . $this->getEndpoint() . '/send', ['json' => $this->getPayload($email, $envelope), 'headers' => ['Api-Key' => $this->apiKey]]);
        try {
    ...
    private function getPayload(Email $email, Envelope $envelope): array
    {
        ...
        $payload = ['recipients' => $this->formatAddresses($this->getRecipients($email, $envelope)), 'from' => $this->formatAddress($envelope->getSender()), 'subject' => $email->getSubject(), 'campaign-type' => 'transac', 'channel' => 'email'];
    ...
    private function getAttachments(Email $email): array
    {
        ...
        foreach ($email->getAttachments() as $attachment) {
            ...
            $att = ['content' => $attachment->bodyToString(), 'filename' => $filename, 'disposition' => $disposition];
    ...
    private function prepareHeaders(Headers $headers): array
    {
        ...
        foreach ($headers->all() as $header) {
            ...
            if (\in_array($header->getName(), ['To', 'From', 'Subject'], \true)) {
                continue;
            }
