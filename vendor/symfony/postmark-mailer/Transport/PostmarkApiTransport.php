 ...
 */
namespace JooosiMailDeps\Symfony\Component\Mailer\Bridge\Postmark\Transport;

use JooosiMailDeps\Psr\EventDispatcher\EventDispatcherInterface;
use JooosiMailDeps\Psr\Log\LoggerInterface;
use JooosiMailDeps\Symfony\Component\Mailer\Bridge\Postmark\Event\PostmarkDeliveryEvent;
...
class PostmarkApiTransport extends AbstractApiTransport
{
    ...
    {
        $response = $this->client->request('POST', 'https://' . $this->getEndpoint() . '/email', ['headers' => ['Accept' => 'application/json', 'X-Postmark-Server-Token' => $this->key], 'json' => $this->getPayload($email, $envelope)]);
        try {
    ...
    private function getPayload(Email $email, Envelope $envelope): array
    {
        ...
        foreach ($email->getHeaders()->all() as $name => $header) {
            if (\in_array($name, ['from', 'to', 'cc', 'bcc', 'subject', 'content-type', 'sender', 'reply-to', 'date'], \true)) {
                continue;
            }
            ...
            $payload['Headers'][] = ['Name' => $header->getName(), 'Value' => $header->getBodyAsString()];
        }
    ...
    private function getAttachments(Email $email): array
    {
        ...
        foreach ($email->getAttachments() as $attachment) {
            ...
            $att = ['Name' => $filename, 'Content' => $attachment->bodyToString(), 'ContentType' => $headers->get('Content-Type')->getBody()];
