 ...
 */
namespace JooosiMailDeps\Symfony\Component\Mailer\Bridge\Postmark\RemoteEvent;
...
final class PostmarkPayloadConverter implements PayloadConverterInterface
{
    ...
    {
        if (\in_array($payload['RecordType'], ['Delivery', 'Bounce'], \true)) {
        ...
        $date = (\DateTimeImmutable::createFromFormat(\DateTimeInterface::ATOM, $payloadDate) ?: \DateTimeImmutable::createFromFormat('Y-m-d\TH:i:s.uP', $payloadDate)) ?: \DateTimeImmutable::createFromFormat('Y-m-d\TH:i:s.u?P', $payloadDate);
        if (!$date) {
