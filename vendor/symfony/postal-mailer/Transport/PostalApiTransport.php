 ...
 */
namespace JooosiMailDeps\Symfony\Component\Mailer\Bridge\Postal\Transport;
...
final class PostalApiTransport extends AbstractApiTransport
{
    ...
    {
        $response = $this->client->request('POST', 'https://' . $this->getEndpoint() . '/api/v1/send/message', ['json' => $this->getPayload($email, $envelope), 'headers' => ['X-Server-API-Key' => $this->apiToken]]);
        try {
    ...
    {
        $payload = ['from' => $envelope->getSender()->getAddress(), 'to' => array_map(static fn(Address $address) => $address->getAddress(), $this->getRecipients($email, $envelope)), 'subject' => $email->getSubject()];
    ...
    private function prepareAttachments(Email $email): array
    {
        ...
        foreach ($email->getAttachments() as $attachment) {
            $attachments[] = ['name' => $attachment->getFilename(), 'content_type' => $attachment->getContentType(), 'data' => base64_encode($attachment->getBody())];
        }
        return $attachments;
    }
