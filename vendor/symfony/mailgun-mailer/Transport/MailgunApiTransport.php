 ...
 */
namespace JooosiMailDeps\Symfony\Component\Mailer\Bridge\Mailgun\Transport;
...
use JooosiMailDeps\Symfony\Component\Mime\Part\Multipart\FormDataPart;
...
class MailgunApiTransport extends AbstractApiTransport
{
    ...
    protected function doSendApi(SentMessage $sentMessage, Email $email, Envelope $envelope): ResponseInterface
    {
        ...
        $response = $this->client->request('POST', 'https://' . $endpoint, ['http_version' => '1.1', 'auth_basic' => 'api:' . $this->key, 'headers' => $headers, 'body' => $body->bodyToIterable()]);
        try {
    ...
    private function getPayload(Email $email, Envelope $envelope): array
    {
        ...
        $payload = ['from' => $envelope->getSender()->toString(), 'to' => implode(',', $this->stringifyAddresses($this->getRecipients($email, $envelope))), 'subject' => $email->getSubject(), 'attachment' => $attachments, 'inline' => $inlines];
        ...
        foreach ($headers->all() as $name => $header) {
            if (\in_array($name, ['from', 'to', 'cc', 'bcc', 'subject', 'content-type'], \true)) {
                continue;
            }
            ...
            if (\in_array($prefix, ['h:', 't:', 'o:', 'v:']) || \in_array($name, ['recipient-variables', 'template', 'amp-html'], \true)) {
