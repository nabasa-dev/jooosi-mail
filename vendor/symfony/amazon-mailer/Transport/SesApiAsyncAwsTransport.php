...
class SesApiAsyncAwsTransport extends SesHttpAsyncAwsTransport
{
    ...
    protected function getRequest(SentMessage $message): SendEmailRequest
    {
        ...
        $request = ['FromEmailAddress' => $this->stringifyAddress(self::getSenderFromHeaders($email->getHeaders())), 'Destination' => ['ToAddresses' => $this->stringifyAddresses($this->getRecipients($email, $envelope))], 'Content' => ['Simple' => ['Subject' => ['Data' => $email->getSubject(), 'Charset' => 'utf-8'], 'Body' => []]]];
        ...
        if ($email->getTextBody()) {
            $request['Content']['Simple']['Body']['Text'] = new Content(['Data' => $email->getTextBody(), 'Charset' => $email->getTextCharset()]);
        }
        if ($email->getHtmlBody()) {
            $request['Content']['Simple']['Body']['Html'] = new Content(['Data' => $email->getHtmlBody(), 'Charset' => $email->getHtmlCharset()]);
        }
    ...
    private function getRecipients(Email $email, Envelope $envelope): array
    {
        ...
        return array_filter($envelope->getRecipients(), static fn(Address $address) => !\in_array($address, $emailRecipients, \true));
    }
    private function getCustomHeaders(Headers $headers): array
    {
        ...
        foreach ($headers->all() as $name => $header) {
            ...
            $headersPrepared[] = ['Name' => $header->getName(), 'Value' => $value];
        }
    ...
    protected function stringifyAddress(Address $a): string
    {
        ...
        if (preg_match('~[\x00-\x08\x10-\x19\x7F-\xFF\r\n]~', $name = $a->getName())) {
            return \sprintf('=?UTF-8?B?%s?= <%s>', base64_encode($name), $a->getEncodedAddress());
        }
        return $a->toString();
    }
