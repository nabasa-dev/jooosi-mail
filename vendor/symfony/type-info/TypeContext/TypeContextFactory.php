...
final class TypeContextFactory
{
     ...
     */
    public function __construct(private readonly ?StringTypeResolver $stringTypeResolver = null, private readonly array $extraTypeAliases = [])
    {
    }
    ...
    public function createFromReflection(\Reflector $reflection): ?TypeContext
    {
        ...
        $typeContext = new TypeContext($typeContext->calledClassName, $typeContext->declaringClassName, $typeContext->namespace, $typeContext->uses, $templates);
        return new TypeContext($typeContext->calledClassName, $typeContext->declaringClassName, $typeContext->namespace, $typeContext->uses, $typeContext->templates, $this->collectTypeAliases($classReflection, $typeContext));
    }
    private function createNewInstanceFromClassName(string $calledClassName, string $declaringClassName): TypeContext
    {
        ...
        $typeContext = new TypeContext($typeContext->calledClassName, $typeContext->declaringClassName, $typeContext->namespace, $typeContext->uses, $this->collectTemplates($calledClassNameReflection, $calledClassTypeContext) + $this->collectTemplates($declaringClassReflection, $typeContext));
        return new TypeContext($typeContext->calledClassName, $typeContext->declaringClassName, $typeContext->namespace, $typeContext->uses, $typeContext->templates, $this->collectTypeAliases($declaringClassReflection, $typeContext));
    }
    private function createBaseTypeContext(string $calledClassName, \ReflectionClass $declaringClassReflection): TypeContext
    {
        ...
        return $this->baseTypeContextCache[$declaringClassName][$calledClassName] ??= new TypeContext($calledClassName, $declaringClassReflection->getName(), trim($declaringClassReflection->getNamespaceName(), '\\'), $this->collectUses($declaringClassReflection));
    }
    /**
