...
final class BootDiscovery
{
    ...
    public function __construct(private readonly ContainerInterface $container, private readonly DiscoveryConfig $config, private readonly DiscoveryCache $cache = new DiscoveryCache(DiscoveryCacheStrategy::NONE))
    {
    }
    /**
    ...
    public function build(?array $discoveryClasses = null, ?array $discoveryLocations = null): array
    {
        ...
        if ($discoveryClasses === null) {
            ...
            $discoveries = array_map($this->resolveDiscovery(...), $this->config->classes);
        ...
        $discoveries = array_map($this->resolveDiscovery(...), $discoveryClasses);
        $this->discover($discoveries, $discoveryLocations);
        return $discoveries;
    }
    /**
    ...
    private function restoreFromCache(array $discoveries, DiscoveryLocation $location): bool
    {
        ...
        foreach ($discoveries as $discovery) {
            $discovery->setItems($discovery->getItems()->addForLocation($location, $cachedForLocation[$discovery::class]));
        }
        return \true;
    }
    private function isCachedLocationUsable(array $discoveries, ?array $cachedForLocation): bool
    {
        return is_array($cachedForLocation) && array_all(array: $discoveries, callback: static fn(Discovery $discovery): bool => array_key_exists($discovery::class, $cachedForLocation) && is_iterable($cachedForLocation[$discovery::class]));
    }
    /**
        ...
        if ($extension === 'php' && ucfirst($fileName) === $fileName) {
            ...
            if ($input instanceof ClassReflector) {
                ...
                if ($skipDiscovery !== null && $skipDiscovery->except === []) {
                    $this->shouldSkipForClass[$resolvedClassName] = \true;
                    ...
                    foreach ($skipDiscovery->except as $except) {
                        $this->shouldSkipForClass[$resolvedClassName][$except] = \true;
                    }
                }
    ...
    private function applyDiscovery(Discovery $discovery): void
    {
        ...
        $discovery->apply();
        $this->appliedDiscovery[$discovery::class] = \true;
    }
    /**
    ...
    private function isLocationCached(DiscoveryLocation $location): bool
    {
        ...
        return match ($this->cache->strategy) {
            ...
            DiscoveryCacheStrategy::FULL => \true,
