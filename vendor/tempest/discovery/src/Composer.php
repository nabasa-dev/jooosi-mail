...
final class Composer
{
    ...
    public function __construct(private readonly string $root, private ?ProcessExecutor $executor = null)
    {
    }
    public function load(): self
    {
        ...
        $this->namespaces = (new ImmutableArray([$this->mainNamespace, ...$this->namespaces]))->filter()->unique(fn(Psr4Namespace $ns) => "{$ns->namespace}:{$ns->path}")->toArray();
    ...
    {
        return (new ImmutableArray($this->composer))->get($path, default: new ImmutableArray())->flatMap(fn(string|iterable $paths, string $namespace) => Arr\map(Arr\wrap($paths), fn(string $path) => new Psr4Namespace($namespace, $path)))->sortByCallback(fn(Psr4Namespace $ns1, Psr4Namespace $ns2) => strlen($ns1->path) <=> strlen($ns2->path))->values()->toArray();
    }
}
