...
final class GenerateDiscoveryCache
{
     ...
     */
    public function __invoke(ContainerInterface $container, DiscoveryConfig $config, DiscoveryCache $cache, ?array $discoveryClasses = null, ?array $discoveryLocations = null, ?array $discoveries = null): void
    {
        $originalStrategy = $cache->strategy;
        $cache = $cache->withStrategy(DiscoveryCacheStrategy::NONE);
        $discoveries ??= (new BootDiscovery(container: $container, config: $config, cache: $cache))->build($discoveryClasses, $discoveryLocations);
