...
{
    use HasAttributes;
    public function __construct(private PHPReflectionMethod $reflectionMethod)
    {
    }
    ...
    {
        $parameter = array_find(array: $this->reflectionMethod->getParameters(), callback: fn(ReflectionParameter $parameter) => $parameter->getName() === $key || $parameter->getPosition() === $key);
        if ($parameter === null) {
            return null;
        }
    ...
    {
        return ['class' => $this->reflectionMethod->getDeclaringClass()->getName(), 'method' => $this->reflectionMethod->getName()];
    }
    public function __unserialize(array $data): void
    {
        $this->reflectionMethod = new PHPReflectionMethod(objectOrMethod: $data['class'], method: $data['method']);
    }
}
