...
final class ClassReflector implements Reflector
{
    ...
    {
        return array_map(fn(PHPReflectionClass $interface) => new TypeReflector($interface), $this->reflectionClass->getInterfaces());
    }
    /** @return PropertyReflector[] */
    public function getPublicProperties(): array
    {
        return array_map(fn(PHPReflectionProperty $property) => new PropertyReflector($property), $this->reflectionClass->getProperties(PHPReflectionProperty::IS_PUBLIC));
    }
    /** @return PropertyReflector[] */
    public function getProperties(): array
    {
        return array_map(fn(PHPReflectionProperty $property) => new PropertyReflector($property), $this->reflectionClass->getProperties());
    }
    /** @return \Tempest\Reflection\MethodReflector[] */
    public function getPublicMethods(): array
    {
        return $this->memoize('public_methods', fn() => array_map(fn(PHPReflectionMethod $method) => new MethodReflector($method), $this->reflectionClass->getMethods(PHPReflectionMethod::IS_PUBLIC)));
    }
    ...
    {
        return is_a($this->name, $className, allow_string: \true);
    }
    public function implements(string $interface): bool
    {
        return $this->isInstantiable() && is_a($this->name, $interface, allow_string: \true);
    }
