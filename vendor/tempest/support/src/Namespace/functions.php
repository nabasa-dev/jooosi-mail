...
function to_fqcn(Stringable|string $path, Stringable|string|null $root = null): string
{
    ...
    return (new ImmutableString($path))->afterLast(['\\', '/'])->stripEnd('.php')->pascal()->prepend('\\')->prepend($namespace)->toString();
}
/**
...
function to_psr4_namespace(Psr4Namespace|array $namespaces, Stringable|string $path, Stringable|string|null $root = null): string
{
    ...
    foreach (Arr\wrap($namespaces) as $namespace) {
        ...
        if ($relativePath->startsWith($namespacePath)) {
            return (string) $relativePath->replaceStart($namespacePath, $namespace->namespace)->replace(['\/', '//', '/'], '\\')->stripEnd('.php')->stripEnd('\\');
        }
    }
...
{
    return (new ImmutableString($path))->replace(['/', '\\'], '/')->stripEnd('/')->stripEnd('.php')->afterLast('/')->classBasename()->toString();
}
/**
...
function prepare_namespace(Stringable|string $path, Stringable|string|null $root = null): ImmutableString
{
    ...
    if ($normalized->endsWith('.php')) {
        return $normalized->contains(['/', '\\']) ? $normalized->beforeLast(['/', '\\']) : new ImmutableString();
    }
    return $normalized;
}
