 ...
 */
function chunk(iterable $array, int $size, bool $preserveKeys = \true): array
{
    $array = to_array($array);
    if ($size <= 0) {
        return [];
...
function random(iterable $array, int $number = 1, bool $preserveKey = \false): mixed
{
    ...
    $randomValues = [];
    foreach ($keys as $key) {
        $preserveKey ? $randomValues[$key] = $array[$key] : $randomValues[] = $array[$key];
    }
    if ($preserveKey === \false) {
        $randomValues = shuffle($randomValues);
    }
    return count($randomValues) > 1 ? new ImmutableArray($randomValues) : $randomValues[0];
}
/**
...
function unique(iterable $array, Closure|string|null $key = null, bool $shouldBeStrict = \false): array
{
    ...
    foreach ($array as $item) {
        ...
        $filterValue = match ($key instanceof Closure) {
            \true => $key($item, $array),
            \false => is_array($item) ? get_by_key($item, $key) : $item,
        };
...
{
    $result = [];
    $filter ??= static fn(mixed $value, mixed $_) => !in_array($value, [\false, null], strict: \true);
...
function get_by_key(iterable $array, int|string $key, mixed $default = null): mixed
{
    ...
    if (isset($value[$key])) {
        return is_array($value[$key]) ? new ImmutableArray($value[$key]) : $value[$key];
    }
...
function flatten(iterable $array, int|float $depth = \INF): array
{
    ...
    foreach ($array as $item) {
        ...
        $values = $depth === 1 ? namespace\values($item) : namespace\flatten($item, $depth - 1);
        foreach ($values as $value) {
            $result[] = $value;
        }
    }
