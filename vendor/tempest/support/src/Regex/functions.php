...
function get_matches(Stringable|string $subject, Stringable|string $pattern, bool $global = \false, int $flags = 0, int $offset = 0): array
{
    ...
    return call_preg($global ? 'preg_match_all' : 'preg_match', static function () use ($subject, $pattern, $global, $flags, $offset): array {
        $matches = [];
        $result = match ($global) {
            \true => preg_match_all((string) $pattern, (string) $subject, $matches, $flags, $offset),
            \false => preg_match((string) $pattern, (string) $subject, $matches, $flags, $offset),
        };
 ...
 */
function get_all_matches(Stringable|string $subject, Stringable|string $pattern, Stringable|string|int|array $matches = 0, int $offset = 0): array
{
    /** @var list<array<int|string, string>> $result */
    $result = get_matches($subject, $pattern, \true, \PREG_SET_ORDER, $offset);
    return arr($result)->map(fn(array $result) => filter($result, fn($_, string|int $key) => in_array($key, wrap($matches), strict: \false)))->toArray();
}
/**
...
function get_match(Stringable|string $subject, Stringable|string $pattern, array|Stringable|int|string|null $match = null, mixed $default = null, int $flags = 0, int $offset = 0): mixed
{
    ...
    if (is_array($match)) {
        return arr($result)->filter(fn($_, string|int $key) => in_array($key, $match, strict: \false))->mapWithKeys(fn(array $matches, string|int $key) => yield $key => first($matches))->toArray();
    }
...
function replace(array|string $haystack, array|string $pattern, Closure|array|string $replacement, ?int $limit = null): string
{
    if ($replacement instanceof Closure) {
        return (string) call_preg('preg_replace_callback', static fn(): ?string => preg_replace_callback($pattern, $replacement, $haystack, $limit ?? -1));
    }
    return (string) call_preg('preg_replace', static fn(): ?string => preg_replace($pattern, $replacement, $haystack, $limit ?? -1));
}
/**
...
{
    return (string) call_preg('preg_replace', static fn(): ?string => preg_replace(array_keys($replacements), array_values($replacements), $haystack, $limit ?? -1));
}
/**
