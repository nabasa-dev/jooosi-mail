...
class SQLServerPlatform extends AbstractPlatform
{
    ...
    protected function _getCreateTableSQL(string $name, array $columns, array $options = []): array
    {
        ...
        foreach ($columns as &$column) {
            ...
            if (isset($column['default'])) {
                $defaultConstraintsSql[] = 'ALTER TABLE ' . $name . ' ADD' . $this->getDefaultConstraintDeclarationSQL($column);
            }
    ...
    public function getAlterTableSQL(TableDiff $diff): array
    {
        ...
        foreach ($diff->getChangedColumns() as $columnDiff) {
            ...
            if ($nameChanged) {
                ...
                $sql = array_merge($sql, $this->getRenameColumnSQL($tableNameSQL, $oldColumnNameSQL, $newColumnName));
            }
            ...
            if ($newColumn->getDefault() === null || !$requireDropDefaultConstraint && !$defaultChanged) {
                continue;
            }
    ...
    public function appendLockHint(string $fromClause, LockMode $lockMode): string
    {
        return match ($lockMode) {
            LockMode::NONE, LockMode::OPTIMISTIC => $fromClause,
    ...
    protected function getCommentOnTableSQL(string $tableName, string $comment): string
    {
        ...
        return $this->getAddExtendedPropertySQL('MS_Description', $comment, 'SCHEMA', $this->quoteStringLiteral($schemaName), 'TABLE', $this->quoteStringLiteral($this->unquoteSingleIdentifier($tableName)));
    }
