...
use JooosiMailDeps\Doctrine\DBAL\Platforms\SQLite\SQLiteMetadataProvider\ForeignKeyConstraintDetails;
...
use JooosiMailDeps\Doctrine\DBAL\Schema\Exception\UnsupportedSchema;
...
, $this->buildTableQueryPredicate($tableName, $params));
    ...
    private function createTableColumn(array $row, array $sqlByTableName): TableColumnMetadataRow
    {
        ...
        $editor = Column::editor()->setQuotedName($columnName);
        $dbType = strtolower($matches[1]);
        if (str_contains($dbType, ' unsigned')) {
            $dbType = str_replace(' unsigned', '', $dbType);
            $editor->setUnsigned(\true);
        }
        ...
        if (isset($matches[2])) {
            if (isset($matches[3])) {
                $editor->setPrecision((int) $matches[2])->setScale((int) $matches[3]);
            } else {
                $editor->setLength((int) $matches[2]);
            }
        }
        if ($defaultExpression !== null) {
            $editor->setDefaultValue($this->parseDefaultExpression($defaultExpression));
        }
        $tableSQL = $sqlByTableName[$tableName];
        $editor->setAutoincrement($this->parseColumnAutoIncrementFromSQL($columnName, $tableSQL))->setComment($this->parseColumnCommentFromSQL($columnName, $tableSQL))->setNotNull((bool) $notNull);
        if ($typeName === Types::STRING || $typeName === Types::TEXT) {
            $editor->setCollation($this->parseColumnCollationFromSQL($columnName, $tableSQL) ?? 'BINARY');
        }
    ...
    private function buildIdentifierPattern(string $identifier): string
    {
        ...
        }, [$identifier, $this->platform->quoteSingleIdentifier($identifier)])) . ')';
    }
    /** {@inheritDoc} */
...
, $this->buildTableQueryPredicate($tableName, $params));
        foreach ($this->connection->iterateNumeric($sql, $params) as $row) {
            yield new IndexColumnMetadataRow(schemaName: null, tableName: $row[0], indexName: $row[1], type: $row[2] ? IndexType::UNIQUE : IndexType::REGULAR, isClustered: \false, predicate: null, columnName: $row[3], columnLength: null);
        }
    }
    /** {@inheritDoc} */
...
, $this->buildTableQueryPredicate($tableName, $params));
        foreach ($this->connection->iterateNumeric($sql, $params) as $row) {
            yield new PrimaryKeyConstraintColumnRow(schemaName: null, tableName: $row[0], constraintName: null, isClustered: \true, columnName: $row[1]);
        }
    }
    /** {@inheritDoc} */
...
SQL
, $this->buildTableQueryPredicate($tableName, $params));
        return $this->generateForeignKeyConstraintColumns($this->connection->iterateNumeric($sql, $params));
    }
    /**
        ...
        foreach ($rows as $row) {
            ...
            if ($row[6] !== null) {
                ...
                if (count($referencedColumnNames) === 0) {
                    throw UnsupportedSchema::sqliteMissingForeignKeyConstraintReferencedColumns($name, $tableName, $row[2]);
                }
            }
            foreach ($referencedColumnNames as $referencedColumnName) {
                yield new ForeignKeyConstraintColumnMetadataRow(referencingSchemaName: null, referencingTableName: $tableName, id: $row[1], name: $name, referencedSchemaName: null, referencedTableName: $row[2], matchType: MatchType::SIMPLE, onUpdateAction: $this->createReferentialAction($row[3]), onDeleteAction: $this->createReferentialAction($row[4]), isDeferrable: $details->isDeferrable(), isDeferred: $details->isDeferred(), referencingColumnName: $row[5], referencedColumnName: $referencedColumnName);
            }
        }
    }
    ...
    private function getForeignKeyConstraintDetails(string $tableName): array
    {
        ...
        for ($i = 0, $count = count($matches[0]); $i < $count; $i++) {
            $details[] = new ForeignKeyConstraintDetails($this->parseOptionallyQuotedName($names[$i]), strcasecmp($deferrable[$i], 'deferrable') === 0, strcasecmp($deferred[$i], 'deferred') === 0);
        }
        return $details;
    }
    /** @return ?non-empty-string */
    ...
    {
        $sql = $this->connection->fetchOne(<<<'SQL'
SELECT sql
FROM sqlite_master
WHERE type = 'table'
  AND name = ?
...
, [$tableName]);
    ...
    private function getTableOptions(iterable $tableNames): iterable
    {
        foreach ($tableNames as $tableName) {
            yield new TableMetadataRow(null, $tableName, ['comment' => $this->parseTableCommentFromSQL($tableName, $this->getCreateTableSQL($tableName))]);
        }
    }
    /**
    ...
    {
        $conditions = ["t.type = 'table'", $this->buildTableNamePredicate('t.name')];
        if ($tableName !== null) {
            $conditions[] = 't.name = ?';
            $params[] = $tableName;
        }
