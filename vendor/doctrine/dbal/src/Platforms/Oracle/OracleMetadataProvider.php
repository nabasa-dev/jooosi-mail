...
namespace JooosiMailDeps\Doctrine\DBAL\Platforms\Oracle;
...
, $this->buildTableQueryPredicate('C', $tableName, $params));
    ...
    {
        [$tableName, $columnName, $dataType, $dataDefault, $dataPrecision, $dataScale, $characterLength, $dataLength, $nullable, $comments] = $row;
        ...
        switch ($dbType) {
            ...
            case 'nvarchar2':
                ...
                break;
            case 'raw':
                $editor->setLength((int) $dataLength)->setFixed(\true);
                break;
            case 'char':
            case 'nchar':
                $editor->setLength((int) $characterLength)->setFixed(\true);
                break;
        }
        $editor->setTypeName($type)->setPrecision($precision)->setScale($scale)->setNotNull($nullable === 'N')->setDefaultValue($this->parseDefaultExpression($dataDefault));
        if ($comments !== null) {
...
, $this->buildTableQueryPredicate('I', $tableName, $params));
        foreach ($this->connection->iterateNumeric($sql, $params) as $row) {
            yield new IndexColumnMetadataRow(schemaName: null, tableName: $row[0], indexName: $row[1], type: $row[2] === 'UNIQUE' ? IndexType::UNIQUE : IndexType::REGULAR, isClustered: \false, predicate: null, columnName: $row[3], columnLength: null);
        }
    }
    /** {@inheritDoc} */
...
SQL;
        foreach ($this->connection->iterateNumeric($sql) as $row) {
            $cacheSize = (int) $row[3];
            assert($cacheSize > 0);
            yield new SequenceMetadataRow(schemaName: null, sequenceName: $row[0], allocationSize: (int) $row[1], initialValue: (int) $row[2], cacheSize: $cacheSize);
        }
    }
}
