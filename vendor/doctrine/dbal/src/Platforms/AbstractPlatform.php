...
use JooosiMailDeps\Doctrine\DBAL\Exception\InvalidColumnDeclaration;
...
use JooosiMailDeps\Doctrine\DBAL\Exception\InvalidColumnType\ColumnPrecisionRequired;
use JooosiMailDeps\Doctrine\DBAL\Exception\InvalidColumnType\ColumnScaleRequired;
use JooosiMailDeps\Doctrine\DBAL\Exception\InvalidColumnType\ColumnValuesRequired;
use JooosiMailDeps\Doctrine\DBAL\LockMode;
use JooosiMailDeps\Doctrine\DBAL\Platforms\Exception\NoColumnsSpecifiedForTable;
...
use JooosiMailDeps\Doctrine\DBAL\Schema\AbstractSchemaManager;
...
use JooosiMailDeps\Doctrine\DBAL\Schema\SchemaDiff;
...
use JooosiMailDeps\Doctrine\DBAL\Schema\UniqueConstraint;
use JooosiMailDeps\Doctrine\DBAL\SQL\Builder\DefaultSelectSQLBuilder;
use JooosiMailDeps\Doctrine\DBAL\SQL\Builder\DefaultUnionSQLBuilder;
use JooosiMailDeps\Doctrine\DBAL\SQL\Builder\SelectSQLBuilder;
use JooosiMailDeps\Doctrine\DBAL\SQL\Builder\UnionSQLBuilder;
...
abstract class AbstractPlatform
{
    ...
    public function getEnumDeclarationSQL(array $column): string
    {
        ...
        $length = count($column['values']) > 1 ? max(...array_map(mb_strlen(...), $column['values'])) : mb_strlen($column['values'][key($column['values'])]);
    ...
    public function getDoctrineTypeMapping(string $dbType): string
    {
        ...
        if (!isset($this->doctrineTypeMapping[$dbType])) {
            throw new InvalidArgumentException(sprintf('Unknown database type "%s" requested, %s may not support it.', $dbType, static::class));
        }
    ...
    {
        return $this->buildCreateTableSQL($table, \true);
    }
    ...
    protected function getCommentOnTableSQL(string $tableName, string $comment): string
    {
        ...
        return sprintf('COMMENT ON TABLE %s IS %s', $tableName->getQuotedName($this), $this->quoteStringLiteral($comment));
    }
    /** @internal The method should be only used from within the {@see AbstractPlatform} class hierarchy. */
    ...
    final protected function validateCreateTableOptions(array $options, string $methodName): void
    {
        ...
        Deprecation::trigger('doctrine/dbal', 'https://github.com/doctrine/dbal/pull/6805', 'Not passing $options or any of its following keys to %s() is deprecated:' . ' "primary", "indexes", "uniqueConstraints", "foreignKeys".', $methodName);
    }
    ...
    public function getAlterSchemaSQL(SchemaDiff $diff): array
    {
        ...
        $sql = array_merge($sql, $this->getCreateTablesSQL($diff->getCreatedTables()), $this->getDropTablesSQL($diff->getDroppedTables()));
...
, __METHOD__);
    ...
    public function getIndexDeclarationSQL(Index $index): string
    {
        ...
        return $this->getCreateIndexSQLFlags($index) . 'INDEX ' . $index->getQuotedName($this) . ' (' . implode(', ', $index->getQuotedColumns($this)) . ')' . $this->getPartialIndexSQL($index);
    }
    /**
    ...
    {
        Deprecation::triggerIfCalledFromOutside('doctrine/dbal', 'https://github.com/doctrine/dbal/pull/6886', '%s is deprecated.', __METHOD__);
        return \false;
    }
    /**
    ...
    {
        $sql = preg_replace('~([' . preg_quote($this->getLikeWildcardCharacters() . $escapeChar, '~') . '])~u', addcslashes($escapeChar, '\\') . '$1', $inputString);
    ...
    {
        return array_merge($column->toArray(), ['name' => $column->getQuotedName($this), 'version' => $column->hasPlatformOption('version') ? $column->getPlatformOption('version') : \false, 'comment' => $column->getComment()]);
    }
    /** @internal */
