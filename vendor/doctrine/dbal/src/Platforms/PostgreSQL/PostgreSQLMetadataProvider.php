...
namespace JooosiMailDeps\Doctrine\DBAL\Platforms\PostgreSQL;
...
final readonly class PostgreSQLMetadataProvider implements MetadataProvider
{
    ...
    private const REFERENTIAL_ACTIONS = ['a' => ReferentialAction::NO_ACTION, 'c' => ReferentialAction::CASCADE, 'd' => ReferentialAction::SET_DEFAULT, 'n' => ReferentialAction::SET_NULL, 'r' => ReferentialAction::RESTRICT];
...
ORDER BY n.nspname,
         c.relname
SQL
, $this->buildNamespaceNamePredicate('n.nspname'), $this->buildTablePredicate('c.relkind', 'c.relname'));
...
ORDER BY n.nspname,
         ...
         a.attnum
SQL
, $this->platform->getDefaultColumnValueSQLSnippet(), $this->buildTableQueryPredicate('n', $schemaName, 'c', $tableName, $params));
    ...
    {
        [$namespaceName, $relationName, $attributeName, $typeName, $completeType, $domainTypeName, $domainCompleteType, $isNotNull, $identity, $defaultExpression, $description, $collationName] = $row;
        $editor = Column::editor()->setQuotedName($attributeName);
        ...
        $editor->setTypeName($this->platform->getDoctrineTypeMapping($typeName));
        switch ($typeName) {
            case 'bpchar':
            case 'varchar':
        ...
        $editor->setNotNull((bool) $isNotNull)->setDefaultValue($this->parseDefaultExpression($defaultExpression))->setAutoincrement($identity === 'd');
        if ($description !== null) {
...
, $this->buildTableQueryPredicate('n', $schemaName, 'c', $tableName, $params));
        foreach ($this->connection->iterateNumeric($sql, $params) as $row) {
            yield new IndexColumnMetadataRow(schemaName: $row[0], tableName: $row[1], indexName: $row[2], type: $row[3] ? IndexType::UNIQUE : IndexType::REGULAR, isClustered: \false, predicate: $row[4], columnName: $row[5], columnLength: null);
        }
    }
    /** {@inheritDoc} */
...
, $this->buildTableQueryPredicate('n', $schemaName, 'c', $tableName, $params));
        foreach ($this->connection->iterateNumeric($sql, $params) as $row) {
            yield new PrimaryKeyConstraintColumnRow(schemaName: $row[0], tableName: $row[1], constraintName: $row[2], isClustered: \true, columnName: $row[3]);
        }
    }
    /** {@inheritDoc} */
...
, $this->buildTableQueryPredicate('pkn', $schemaName, 'pkc', $tableName, $params));
        foreach ($this->connection->iterateNumeric($sql, $params) as $row) {
            yield new ForeignKeyConstraintColumnMetadataRow($row[0], $row[1], null, $row[2], $row[3], $row[4], MatchType::SIMPLE, self::REFERENTIAL_ACTIONS[$row[5]], self::REFERENTIAL_ACTIONS[$row[6]], (bool) $row[7], (bool) $row[8], $row[9], $row[10]);
        }
    }
    /** {@inheritDoc} */
...
, $this->buildTableQueryPredicate('n', $schemaName, 'c', $tableName, $params));
        foreach ($this->connection->iterateNumeric($sql, $params) as $row) {
            yield new TableMetadataRow($row[0], $row[1], ['unlogged' => $row[2], 'comment' => $row[3]]);
        }
    }
    /**
     ...
     */
    private function buildTableQueryPredicate(string $namespaceRelation, ?string $schemaName, string $tableRelation, ?string $tableName, array &$params): string
    {
        $conditions = [];
