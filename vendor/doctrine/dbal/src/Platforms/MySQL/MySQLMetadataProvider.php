...
ORDER BY c.TABLE_NAME,
         c.ORDINAL_POSITION
SQL
, $this->platform->getColumnTypeSQLSnippet('c', $this->databaseName), implode(' AND ', $conditions));
    ...
    {
        [$tableName, $columnName, $dbType, $columnType, $characterMaximumLength, $characterOctetLength, $numericPrecision, $numericScale, $isNullable, $columnDefault, $extra, $columnComment, $characterSetName, $collationName] = $row;
        ...
        $editor->setDefaultValue($default)->setNotNull($isNullable !== 'YES')->setComment($columnComment)->setCharset($characterSetName)->setCollation($collationName);
        if (str_contains($extra, 'auto_increment')) {
            $editor->setAutoincrement(\true);
        }
...
, implode(' AND ', $conditions));
        foreach ($this->connection->iterateNumeric($sql, $params) as $row) {
            ...
            yield new IndexColumnMetadataRow(schemaName: null, tableName: $row[0], indexName: $row[1], type: $type, isClustered: \false, predicate: null, columnName: $row[4], columnLength: $length);
        }
    }
    /** {@inheritDoc} */
...
, implode(' AND ', $conditions));
    ...
    private function getTableOptions(?string $tableName): iterable
    {
        ...
        foreach ($this->connection->iterateNumeric($sql, $params) as $row) {
            yield new TableMetadataRow(null, $row[0], ['engine' => $row[1], 'autoincrement' => $row[2], 'comment' => $row[3], 'create_options' => $this->parseCreateOptions($row[4]), 'collation' => $row[5], 'charset' => $row[6]]);
        }
    }
