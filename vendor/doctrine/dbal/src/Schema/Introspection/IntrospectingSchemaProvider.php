...
final readonly class IntrospectingSchemaProvider implements SchemaProvider
{
    ...
    public function __construct(private MetadataProvider $metadataProvider, private ?string $currentSchemaName, private TableConfiguration $tableConfiguration)
    {
    }
    /** {@inheritDoc} */
    ...
    public function getAllTables(): array
    {
        ...
        foreach ($tableColumnsByTable as $schemaNameKey => $schemaTables) {
            ...
            foreach ($schemaTables as $unqualifiedName => $tableColumns) {
                $editor = Table::editor()->setName(OptionallyQualifiedName::quoted($unqualifiedName, $schemaName))->setColumns(...$tableColumns)->setIndexes(...$indexesByTable[$schemaNameKey][$unqualifiedName] ?? []);
                if (isset($primaryKeyConstraintsByTable[$schemaNameKey][$unqualifiedName])) {
                    $editor->setPrimaryKeyConstraint($primaryKeyConstraintsByTable[$schemaNameKey][$unqualifiedName]);
                }
                if (isset($foreignKeyConstraintsByTable[$schemaNameKey][$unqualifiedName])) {
                    $editor->setForeignKeyConstraints(...$foreignKeyConstraintsByTable[$schemaNameKey][$unqualifiedName]);
                }
                ...
                $tables[] = $editor->setConfiguration($this->tableConfiguration)->create();
            }
        }
        return $tables;
    }
    ...
    public function getIndexesForTable(?string $schemaName, string $tableName): array
    {
        ...
        return array_map(static fn(IndexEditor $e): Index => $e->create(), array_values($editors));
    }
    /**
    ...
    private function getIndexesForAllTables(): array
    {
        ...
        return array_map(static fn(array $editors): array => array_map(static fn(array $editors): array => array_map(static fn(IndexEditor $editor): Index => $editor->create(), array_values($editors)), $editors), $editors);
    }
    ...
    private function getPrimaryKeyConstraintsForAllTables(): array
    {
        ...
        return array_map(static fn(array $editors): array => array_map(static fn(PrimaryKeyConstraintEditor $editor): PrimaryKeyConstraint => $editor->create(), $editors), $editors);
    }
    /** {@inheritDoc} */
    public function getForeignKeyConstraintsForTable(?string $schemaName, string $tableName): array
    {
        ...
        return array_map(static fn(ForeignKeyConstraintEditor $e): ForeignKeyConstraint => $e->create(), array_values($editors));
    }
    /**
    ...
    private function getForeignKeyConstraintsForAllTables(): array
    {
        ...
        return array_map(static fn(array $editors): array => array_map(static fn(array $editors): array => array_map(static fn(ForeignKeyConstraintEditor $editor): ForeignKeyConstraint => $editor->create(), array_values($editors)), $editors), $editors);
    }
    /** {@inheritDoc} */
