...
final class UniqueConstraintEditor
{
    ...
    {
        $this->columnNames = array_map(static fn(string $name): UnqualifiedName => UnqualifiedName::unquoted($name), array_merge([$firstColumnName], array_values($otherColumnNames)));
        return $this;
    }
    /**
     * @param non-empty-string $firstColumnName
    ...
    {
        $this->columnNames = array_map(static fn(string $name): UnqualifiedName => UnqualifiedName::quoted($name), array_merge([$firstColumnName], array_values($otherColumnNames)));
        return $this;
    }
