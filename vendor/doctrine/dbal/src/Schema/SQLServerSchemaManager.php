...
use JooosiMailDeps\Doctrine\DBAL\Platforms\SQLServer;
...
SQL
    ...
    protected function _getPortableTableColumnDefinition(array $tableColumn): Column
    {
        ...
        $options = ['fixed' => $fixed, 'notnull' => (bool) $tableColumn['notnull'], 'scale' => $scale, 'precision' => $precision, 'autoincrement' => (bool) $tableColumn['autoincrement']];
        ...
        if ($tableColumn['default'] !== null) {
            ...
            $column->setPlatformOption(SQLServerPlatform::OPTION_DEFAULT_CONSTRAINT_NAME, $tableColumn['df_name']);
        }
    ...
    protected function _getPortableTableForeignKeysList(array $rows): array
    {
        ...
        foreach ($rows as $row) {
            ...
            if (!isset($foreignKeys[$name])) {
                ...
                $foreignKeys[$name] = ['local_columns' => [$row['ColumnName']], 'foreign_table' => $referencedTableName, 'foreign_columns' => [$row['ReferenceColumnName']], 'name' => $name, 'options' => ['onUpdate' => str_replace('_', ' ', $row['update_referential_action_desc']), 'onDelete' => str_replace('_', ' ', $row['delete_referential_action_desc'])]];
            } else {
    ...
    private function getDatabaseCollation(): string
    {
        if ($this->databaseCollation === null) {
            $databaseCollation = $this->connection->fetchOne('SELECT collation_name FROM sys.databases WHERE name = ' . $this->platform->getCurrentDatabaseExpression());
...
SQL
, $this->getWhereClause($tableName, 'SCHEMA_NAME(f.schema_id)', 'OBJECT_NAME(f.parent_object_id)', $params));
        return $this->connection->executeQuery($sql, $params);
    }
    /**
     * {@inheritDoc}
...
, $this->getWhereClause($tableName, 'scm.name', 'tbl.name', $params));
        ...
        foreach ($this->connection->iterateAssociative($sql, $params) as $data) {
            ...
            $tableOptions[$this->_getPortableTableDefinition($data)] = ['comment' => $data['value']];
        }
        return $tableOptions;
    }
    /**
     ...
     */
    private function getWhereClause(?string $tableName, string $schemaColumn, string $tableColumn, array &$params): string
    {
        $conditions = [];
        if ($tableName !== null) {
