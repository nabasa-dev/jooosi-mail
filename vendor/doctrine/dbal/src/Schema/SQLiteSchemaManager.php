    ...
    protected function _getPortableTableForeignKeysList(array $rows): array
    {
        ...
        foreach ($rows as $row) {
            ...
            if (!isset($list[$id])) {
                ...
                $list[$id] = ['name' => $row['constraint_name'], 'local' => [], 'foreign' => [], 'foreignTable' => $row['table'], 'onDelete' => $row['on_delete'], 'onUpdate' => $row['on_update'], 'deferrable' => $row['deferrable'], 'deferred' => $row['deferred']];
            }
        ...
        foreach ($list as $id => $value) {
            ...
            if (count($foreignTablePrimaryKeyColumnRows) < 1) {
                Deprecation::trigger('doctrine/dbal', 'https://github.com/doctrine/dbal/pull/6701', 'Introspection of SQLite foreign key constraints with omitted referenced column names' . ' in an incomplete schema is deprecated.');
                continue;
            }
...
, [$table]);
    ...
    private function getForeignKeyDetails(string $table): array
    {
        ...
        for ($i = 0, $count = count($match[0]); $i < $count; $i++) {
            $details[] = ['constraint_name' => $names[$i] ?? '', 'deferrable' => isset($deferrable[$i]) && strcasecmp($deferrable[$i], 'deferrable') === 0, 'deferred' => isset($deferred[$i]) && strcasecmp($deferred[$i], 'deferred') === 0];
        }
        return $details;
    }
...
, $this->getWhereClause($tableName, $params));
    ...
    protected function fetchTableColumns(string $databaseName, ?string $tableName = null): array
    {
        ...
        foreach ($rows as $row) {
            ...
            $result[] = array_merge($row, ['autoincrement' => isset($pkColumnNamesByTable[$tableName]) && $pkColumnNamesByTable[$tableName] === [$columnName], 'collation' => $this->parseColumnCollationFromSQL($columnName, $tableSQL), 'comment' => $this->parseColumnCommentFromSQL($columnName, $tableSQL)]);
        }
        return $result;
    }
    /**
    ...
    protected function fetchIndexColumns(string $databaseName, ?string $tableName = null): array
    {
        ...
        foreach ($pkColumnNameRows as $pkColumnNameRow) {
            $result[] = ['table_name' => $pkColumnNameRow['table_name'], 'key_name' => 'primary', 'primary' => \true, 'non_unique' => \false, 'column_name' => $pkColumnNameRow['name']];
        }
        $indexColumnRows = parent::fetchIndexColumns($databaseName, $tableName);
        foreach ($indexColumnRows as $indexColumnRow) {
            $result[] = ['table_name' => $indexColumnRow['table_name'], 'key_name' => $indexColumnRow['name'], 'primary' => \false, 'non_unique' => !$indexColumnRow['unique'], 'column_name' => $indexColumnRow['column_name']];
        }
        return $result;
    }
    /**
