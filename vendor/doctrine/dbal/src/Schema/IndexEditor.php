...
final class IndexEditor
{
    ...
    {
        $this->columns = array_map(static fn(UnqualifiedName $name) => new IndexedColumn($name, null), [$firstColumnName, ...array_values($otherColumnNames)]);
        return $this;
    }
    /**
     ...
     */
    public function setUnquotedColumnNames(string $firstColumnName, string ...$otherColumnNames): self
    {
        $this->columns = array_map(static fn(string $name): IndexedColumn => new IndexedColumn(UnqualifiedName::unquoted($name), null), [$firstColumnName, ...array_values($otherColumnNames)]);
        return $this;
    }
    /**
     * @param non-empty-string $firstColumnName
     * @param non-empty-string ...$otherColumnNames
     */
    public function setQuotedColumnNames(string $firstColumnName, string ...$otherColumnNames): self
    {
        $this->columns = array_map(static fn(string $name): IndexedColumn => new IndexedColumn(UnqualifiedName::quoted($name), null), [$firstColumnName, ...array_values($otherColumnNames)]);
        return $this;
    }
