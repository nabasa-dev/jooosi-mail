...
use JooosiMailDeps\Doctrine\DBAL\Schema\Exception\InvalidPrimaryKeyConstraintDefinition;
...
final class PrimaryKeyConstraintEditor
{
    ...
    {
        $this->columnNames = array_map(static fn(string $name): UnqualifiedName => UnqualifiedName::unquoted($name), [$firstColumnName, ...array_values($otherColumnNames)]);
        return $this;
    }
    /**
     * @param non-empty-string $firstColumnName
    ...
    {
        $this->columnNames = array_map(static fn(string $name): UnqualifiedName => UnqualifiedName::quoted($name), [$firstColumnName, ...array_values($otherColumnNames)]);
        return $this;
    }
