...
final class InvalidState extends LogicException implements SchemaException
{
    ...
    {
        return new self(sprintf('Foreign key constraint "%s" has invalid referenced table name.', $constraintName));
    }
    public static function foreignKeyConstraintHasInvalidReferencingColumnNames(string $constraintName): self
    {
        return new self(sprintf('Foreign key constraint "%s" has one or more invalid referencing column names.', $constraintName));
    }
    public static function foreignKeyConstraintHasInvalidReferencedColumnNames(string $constraintName): self
    {
        return new self(sprintf('Foreign key constraint "%s" has one or more invalid referenced column name.', $constraintName));
    }
