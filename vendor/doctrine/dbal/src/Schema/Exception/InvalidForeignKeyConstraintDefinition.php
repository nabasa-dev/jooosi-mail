...
final class InvalidForeignKeyConstraintDefinition extends LogicException implements SchemaException
{
    ...
    {
        return new self(sprintf('Referenced table name is not set for foreign key constraint %s.', self::formatName($constraintName)));
    }
    public static function referencingColumnNamesNotSet(?UnqualifiedName $constraintName): self
    {
        return new self(sprintf('Referencing column names are not set for foreign key constraint %s.', self::formatName($constraintName)));
    }
    public static function referencedColumnNamesNotSet(?UnqualifiedName $constraintName): self
    {
        return new self(sprintf('Referenced column names are not set for foreign key constraint %s.', self::formatName($constraintName)));
    }
