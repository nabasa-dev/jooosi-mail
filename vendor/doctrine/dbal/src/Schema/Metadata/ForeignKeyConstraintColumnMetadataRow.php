...
final readonly class ForeignKeyConstraintColumnMetadataRow
{
     ...
     */
    public function __construct(private ?string $referencingSchemaName, private string $referencingTableName, int|string|null $id, private ?string $name, private ?string $referencedSchemaName, private string $referencedTableName, private MatchType $matchType, private ReferentialAction $onUpdateAction, private ReferentialAction $onDeleteAction, private bool $isDeferrable, private bool $isDeferred, private string $referencingColumnName, private string $referencedColumnName)
    {
        if ($id !== null) {
            $this->id = $id;
        } elseif ($name !== null) {
            ...
            throw new InvalidArgumentException('Either the id or name must be set to a non-null value.');
        }
    }
