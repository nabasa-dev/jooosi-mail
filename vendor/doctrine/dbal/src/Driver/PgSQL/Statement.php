...
final class Statement implements StatementInterface
{
    ...
    public function __construct(private readonly PgSqlConnection $connection, private readonly string $name, private readonly array $parameterMap)
    {
    }
    ...
    public function execute(): Result
    {
        ...
        foreach ($this->parameters as $parameter => $value) {
            $escapedParameters[] = match ($this->parameterTypes[$parameter]) {
                ParameterType::BINARY, ParameterType::LARGE_OBJECT => $value === null ? null : pg_escape_bytea($this->connection, is_resource($value) ? stream_get_contents($value) : $value),
                default => $value,
            };
        }
