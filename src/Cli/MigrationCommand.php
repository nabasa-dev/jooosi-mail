...
final readonly class MigrationCommand
{
     ...
     * @since 0.1.0
     */
    #[Command(name: 'omni-mail migration:make', description: 'Generate an Omni Mail migration stub.', aliases: ['omni-mail migration:create'])]
    ...
    public function status(array $args, array $assocArgs): void
    {
        ...
        $items = [['metric' => 'migration_table', 'value' => $status['migration_table']], ['metric' => 'migrations_directory', 'value' => $status['migrations_directory']], ['metric' => 'previous_version', 'value' => $status['previous_version']], ['metric' => 'current_version', 'value' => $status['current_version']], ['metric' => 'next_version', 'value' => $status['next_version']], ['metric' => 'latest_version', 'value' => $status['latest_version']], ['metric' => 'available_migrations', 'value' => (string) $status['total']], ['metric' => 'executed_migrations', 'value' => (string) $status['executed']], ['metric' => 'executed_unavailable_migrations', 'value' => (string) $status['executed_unavailable']], ['metric' => 'pending_migrations', 'value' => (string) $status['pending']]];
    ...
    public function listing(array $args, array $assocArgs): void
    {
        ...
        $items = array_map(static fn(array $migration): array => ['version' => (string) $migration['version'], 'class_name' => (string) $migration['class_name'], 'description' => (string) $migration['description'], 'status' => (string) $migration['status'], 'executed_at' => (string) ($migration['executed_at'] ?? ''), 'execution_time_ms' => $migration['execution_time_ms'] === null ? '' : (string) $migration['execution_time_ms']], $migrations);
    ...
    public function run(array $args, array $assocArgs): void
    {
        ...
        $items = array_map(static fn(array $migration): array => ['version' => (string) $migration['version'], 'class_name' => (string) $migration['class_name'], 'description' => (string) $migration['description'], 'status' => (string) $migration['status'], 'execution_time_ms' => $migration['execution_time_ms'] === null ? '' : (string) $migration['execution_time_ms']], $result['executed']);
        if ($items !== []) {
        ...
        if (isset($result['failed'])) {
            $failed = [['version' => (string) $result['failed']['version'], 'class_name' => (string) $result['failed']['class_name'], 'description' => (string) $result['failed']['description'], 'status' => (string) $result['failed']['status'], 'error' => (string) $result['failed']['error']]];
            if ($format === 'table') {
                WP_CLI::line('');
            }
    ...
    public function rollback(array $args, array $assocArgs): void
    {
        ...
        try {
            ...
            $result = $reset ? $this->migrationManager->reset($dryRun) : $this->migrationManager->rollback($steps, $toVersion, $dryRun);
        } catch (RuntimeException $exception) {
            WP_CLI::error($exception->getMessage());
            return;
        }
        $items = array_map(static fn(array $migration): array => ['version' => (string) $migration['version'], 'class_name' => (string) $migration['class_name'], 'description' => (string) $migration['description'], 'status' => (string) $migration['status'], 'execution_time_ms' => $migration['execution_time_ms'] === null ? '' : (string) $migration['execution_time_ms']], $result['rolled_back']);
        if ($items !== []) {
            format_items($format, $items, ['version', 'class_name', 'description', 'status', 'execution_time_ms']);
        } elseif ($format !== 'table') {
            format_items($format, [], ['version', 'class_name', 'description', 'status', 'execution_time_ms']);
    ...
    private function resolveStatusFilter(array $assocArgs): string
    {
        ...
        if (!in_array($status, $supportedStatuses, \true)) {
    ...
    private function hasFlag(array $assocArgs, string $flag): bool
    {
        ...
        return in_array(strtolower((string) $value), ['1', 'true', 'yes', 'on'], \true);
    }
    /**
