...
final readonly class WebhookCommand
{
    ...
    public function status(array $args, array $assocArgs): void
    {
        ...
        $connections = array_values(array_filter($this->connectionRepository->findAll(), static fn(Connection $connection): bool => $includeDisabled || $connection->webhookEnabled));
        if ($connections === []) {
        ...
        $items = array_map(function (Connection $connection): array {
            ...
            return ['id' => (string) ($connection->id ?? 0), 'name' => $connection->name, 'profile' => $connection->profileKey, 'enabled' => $connection->webhookEnabled ? 'yes' : 'no', 'adapter' => $this->formatAdapterName($adapter::class), 'verification' => $connection->webhookEnabled ? $adapter->describeVerification($connection) : 'disabled', 'secret' => $connection->hasWebhookSecret() ? 'yes' : 'no'];
        }, $connections);
    ...
    public function events(array $args, array $assocArgs): void
    {
        ...
        $items = array_map(fn(array $row): array => ['id' => (string) (int) ($row['id'] ?? 0), 'connection' => $this->formatConnectionLabel($row), 'mail_log_id' => $this->formatOptionalInt($row['mail_log_id'] ?? null), 'event_type' => (string) ($row['event_type'] ?? ''), 'occurred_at' => (string) ($row['occurred_at'] ?? $row['created_at'] ?? ''), 'transport_message_id' => $this->formatValue((string) ($row['transport_message_id'] ?? '')), 'provider_event_id' => $this->formatValue((string) ($row['provider_event_id'] ?? ''))], $rows);
