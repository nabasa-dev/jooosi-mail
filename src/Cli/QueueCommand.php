...
final readonly class QueueCommand
{
    ...
    {
        $processed = $this->queueWorker->run(limit: max(1, (int) ($assocArgs['limit'] ?? 25)), timeLimit: max(5, (int) ($assocArgs['time-limit'] ?? 20)));
    ...
    public function failed(array $args, array $assocArgs): void
    {
        ...
        $items = array_map(fn(array $row): array => ['id' => (string) (int) $row['id'], 'queue' => (string) ($row['queue_name'] ?? DatabaseTransport::NAME), 'attempts' => (string) (int) ($row['attempt_count'] ?? 0), 'processed_at' => (string) ($row['processed_at'] ?? ''), 'error' => $this->formatError((string) ($row['last_error'] ?? ''))], $rows);
    ...
    private function formatProcessingRow(array $row, int $staleAfter): array
    {
        ...
        return ['id' => (string) (int) ($row['id'] ?? 0), 'queue' => (string) ($row['queue_name'] ?? DatabaseTransport::NAME), 'attempts' => (string) (int) ($row['attempt_count'] ?? 0), 'claimed_at' => $claimedAt, 'claim_age' => $claimAgeSeconds !== null ? sprintf('%ds', $claimAgeSeconds) : 'n/a', 'stale' => $claimAgeSeconds !== null && $claimAgeSeconds >= $staleAfter ? 'yes' : 'no', 'error' => $this->formatError((string) ($row['last_error'] ?? ''))];
    }
    /**
     * @since 0.1.0
     */
