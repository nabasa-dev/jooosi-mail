...
final readonly class ConnectionCommand
{
    ...
    public function enable(array $args, array $assocArgs): void
    {
        $connectionId = $this->resolveConnectionId($args);
        try {
            $connection = $this->connectionManager->setEnabled($connectionId, \true);
    ...
    {
        $items = array_map(static fn($connection): array => ['id' => (string) ($connection->id ?? '-'), 'name' => $connection->name, 'profile' => $connection->profileKey, 'enabled' => $connection->enabled ? 'yes' : 'no', 'default' => $connection->default ? 'yes' : 'no', 'priority' => (string) $connection->priority, 'weight' => (string) $connection->weight, 'webhooks' => $connection->webhookEnabled ? 'yes' : 'no'], $this->connectionRepository->findAll());
        if ($items === []) {
    ...
    {
        $items = array_map(static fn(array $profile): array => ['key' => (string) $profile['key'], 'label' => (string) $profile['label'], 'schemes' => implode(',', $profile['schemes']), 'webhooks' => !empty($profile['supports_webhooks']) ? 'yes' : 'no', 'fields' => implode(',', array_keys((array) $profile['configuration_fields']))], $this->connectionManager->listProfiles());
    ...
    public function status(array $args, array $assocArgs): void
    {
        ...
        $items = array_map(function (array $status): array {
            ...
            return ['id' => (string) ($connection->id ?? '-'), 'name' => $connection->name, 'profile' => $connection->profileKey, 'enabled' => $connection->enabled ? 'yes' : 'no', 'default' => $connection->default ? 'yes' : 'no', 'health' => (string) ($status['health_score'] ?? 0), 'available' => $availability['available'] ?? \false ? 'yes' : 'no', 'reasons' => $this->formatReasons($availability['unavailable_reasons'] ?? []), 'blacklisted_until' => $this->formatTimestamp($availability['blacklisted_until'] ?? null), 'next_available_at' => $this->formatTimestamp($availability['next_available_at'] ?? null), 'rate_limits' => $this->formatRateLimits(is_array($rateLimit) ? $rateLimit : [])];
        }, $statuses);
        format_items('table', $items, ['id', 'name', 'profile', 'enabled', 'default', 'health', 'available', 'reasons', 'blacklisted_until', 'next_available_at', 'rate_limits']);
