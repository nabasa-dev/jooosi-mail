...
final readonly class TestEmailSender
{
    ...
    public function send(string $to, ?string $subject = null, ?int $connectionId = null): bool
    {
        ...
        $bodies = $this->templateRenderer->render(recipientSummary: $to, connectionLabel: __('Not routed by Jooosi Mail', 'jooosi-mail'), connectionProfile: __('WordPress native wp_mail()', 'jooosi-mail'), deliveryMode: __('Native wp_mail', 'jooosi-mail'), routingStrategy: __('None', 'jooosi-mail'), mailLogLabel: __('Not logged by Jooosi Mail', 'jooosi-mail'), mailLogUrl: admin_url('admin.php?page=jooosi-mail#/logs/mail'));
        $markTestMailRequest = static fn(MailRequest $mailRequest): MailRequest => new MailRequest(from: $mailRequest->from, to: $mailRequest->to, cc: $mailRequest->cc, bcc: $mailRequest->bcc, replyTo: $mailRequest->replyTo, subject: $mailRequest->subject, textBody: $mailRequest->textBody, htmlBody: $mailRequest->htmlBody, attachments: $mailRequest->attachments, headers: $mailRequest->headers, envelopeSender: $mailRequest->envelopeSender, source: 'admin_test_email', metadata: array_merge($mailRequest->metadata, [\JooosiMail\Admin\Mail\TestEmailDeliveryTemplateListener::METADATA_KEY => \true]));
