...
final readonly class DashboardController
{
    ...
    public function index(WP_REST_Request $request): WP_REST_Response
    {
        ...
        return new WP_REST_Response(['summary' => ['deliveryMode' => (string) $this->optionStore->get('settings.delivery.mode', 'async'), 'routingStrategy' => (string) $this->optionStore->get('settings.delivery.strategy', 'weighted_random'), 'interceptEnabled' => (bool) $this->optionStore->get('settings.mail.intercept.enabled', \true), 'connectionsTotal' => count($this->connectionStatusReporter->getStatuses(\true)), 'activeConnections' => (int) ($connectionSummary['active_connections'] ?? 0), 'availableConnections' => (int) ($connectionSummary['available_connections'] ?? 0), 'temporarilyUnavailableConnections' => (int) ($connectionSummary['temporarily_unavailable_connections'] ?? 0), 'nextAvailableAt' => $this->normalizeDateTime($connectionSummary['next_available_at'] ?? null), 'queuePendingReady' => (int) ($queueSummary['pending_ready'] ?? 0), 'queuePendingDeferred' => (int) ($queueSummary['pending_deferred'] ?? 0), 'queueProcessing' => (int) ($queueSummary['processing'] ?? 0), 'queueStaleProcessing' => (int) ($queueSummary['stale_processing'] ?? 0), 'queueFailed' => (int) ($queueSummary['failed'] ?? 0), 'mailPending' => (int) ($mailSummary['pending'] ?? 0), 'mailQueued' => (int) ($mailSummary['queued'] ?? 0), 'mailProcessing' => (int) ($mailSummary['processing'] ?? 0), 'mailSent' => (int) ($mailSummary['sent'] ?? 0), 'mailFailed' => (int) ($mailSummary['failed'] ?? 0), 'mailTotal' => (int) ($mailSummary['total'] ?? 0), 'webhookEvents' => $this->getWebhookEventCount()], 'connections' => $this->getConnectionPayloads(), 'sendingStats' => $this->getSendingStats($fromDate, $toDate), 'recentAttempts' => $this->normalizeAttempts($this->mailAttemptRepository->listRecent(8)), 'recentWebhooks' => $this->normalizeWebhookEvents($this->webhookEventRepository->listRecent(8)), 'failedMessages' => $this->normalizeQueueMessages($this->queueMessageQuery->listFailed(5))]);
    }
    /**
    ...
    {
        return array_map(fn(array $attempt): array => ['id' => (int) ($attempt['id'] ?? 0), 'mailLogId' => (int) ($attempt['mail_log_id'] ?? 0), 'connectionId' => (int) ($attempt['connection_id'] ?? 0), 'connectionName' => (string) ($attempt['connection_name'] ?? ''), 'status' => (string) ($attempt['status'] ?? ''), 'errorMessage' => isset($attempt['error_message']) ? (string) $attempt['error_message'] : null, 'transportMessageId' => isset($attempt['transport_message_id']) ? (string) $attempt['transport_message_id'] : null, 'startedAt' => isset($attempt['started_at']) ? (string) $attempt['started_at'] : null, 'finishedAt' => isset($attempt['finished_at']) ? (string) $attempt['finished_at'] : null], $attempts);
    }
    /**
    ...
    private function normalizeRateLimit(array $rateLimit): array
    {
        ...
        foreach ($rateLimit['windows'] ?? [] as $period => $window) {
            ...
            $windows[(string) $period] = ['limit' => (int) ($window['limit'] ?? 0), 'count' => (int) ($window['count'] ?? 0), 'remaining' => isset($window['remaining']) ? (int) $window['remaining'] : null, 'windowStartedAt' => $this->normalizeDateTime($window['window_started_at'] ?? null), 'windowEndsAt' => $this->normalizeDateTime($window['window_ends_at'] ?? null), 'exhausted' => (bool) ($window['exhausted'] ?? \false)];
        }
        return ['blocked' => (bool) ($rateLimit['blocked'] ?? \false), 'windows' => $windows];
    }
    /**
    ...
    {
        return ['enabled' => (bool) ($circuitBreaker['enabled'] ?? \false), 'threshold' => (int) ($circuitBreaker['threshold'] ?? 0), 'windowSeconds' => (int) ($circuitBreaker['window_seconds'] ?? 0), 'cooldownSeconds' => (int) ($circuitBreaker['cooldown_seconds'] ?? 0), 'recentFailures' => (int) ($circuitBreaker['recent_failures'] ?? 0), 'blacklistedUntil' => $this->normalizeDateTime($circuitBreaker['blacklisted_until'] ?? null)];
    }
    /**
     * @since 0.1.0
     */
