...
final readonly class ConnectionController
{
    ...
    public function index(WP_REST_Request $request): WP_REST_Response
    {
        ...
        return new WP_REST_Response(['profiles' => $this->createProfilePayloads(), 'connections' => $connections]);
    }
    /**
     * @since 0.1.0
     */
    ...
    public function show(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        return new WP_REST_Response(['connection' => $this->createConnectionDetailPayload($connection)]);
    }
    /**
     * @since 0.1.0
     */
    ...
    public function delete(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        return new WP_REST_Response(['deleted' => \true]);
    }
    /**
     * @since 0.1.0
     */
    ...
    public function makeDefault(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        return new WP_REST_Response(['connection' => $this->createConnectionDetailPayload($defaultConnection)]);
    }
    /**
     * @since 0.1.0
     */
    ...
    private function createProfilePayloads(): array
    {
        ...
        return array_map(function (array $profile): array {
            ...
            foreach ((array) ($profile['configuration_fields'] ?? []) as $fieldName => $field) {
                ...
                $configurationFields[] = ['name' => (string) $fieldName, 'label' => (string) ($field['label'] ?? $fieldName), 'type' => (string) ($field['type'] ?? 'text'), 'required' => (bool) ($field['required'] ?? \false), 'secret' => ($field['type'] ?? null) === 'password' || ($field['secret'] ?? \false) === \true, 'default' => $field['default'] ?? null, 'choices' => array_values(array_map('strval', is_array($field['choices'] ?? null) ? $field['choices'] : [])), 'visibleWhen' => $this->normalizeFieldConditions($field['visible_when'] ?? null), 'requiredWhen' => $this->normalizeFieldConditions($field['required_when'] ?? null)];
            }
