...
final readonly class SettingsController
{
     ...
     * @since 0.1.0
     */
    public function __construct(private OptionStore $optionStore, private MailLogRetentionService $mailLogRetentionService)
    {
    }
    /**
     * @since 0.1.0
    ...
    public function update(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        $this->optionStore->set(MailLogRetentionPolicy::ENABLED_PATH, (bool) ($emailLogging['enabled'] ?? \true));
    ...
    {
        return ['settings' => ['mail' => ['intercept' => ['enabled' => (bool) $this->optionStore->get('settings.mail.intercept.enabled', \true)], 'sender' => ['email' => (string) $this->optionStore->get('settings.mail.sender.email', ''), 'name' => (string) $this->optionStore->get('settings.mail.sender.name', ''), 'forceEmail' => (bool) $this->optionStore->get('settings.mail.sender.force_email', \false), 'forceName' => (bool) $this->optionStore->get('settings.mail.sender.force_name', \false), 'returnPathMode' => (string) $this->optionStore->get('settings.mail.sender.return_path_mode', SenderPolicyResolver::RETURN_PATH_MODE_PROVIDER_DEFAULT), 'returnPathEmail' => (string) $this->optionStore->get('settings.mail.sender.return_path_email', '')]], 'logging' => ['email' => ['enabled' => (bool) $this->optionStore->get(MailLogRetentionPolicy::ENABLED_PATH, \true), 'retentionDays' => $this->getConfiguredRetentionDays()]], 'delivery' => ['mode' => (string) $this->optionStore->get('settings.delivery.mode', DeliveryMode::Async->value), 'strategy' => (string) $this->optionStore->get('settings.delivery.strategy', RoutingStrategy::WeightedRandom->value)], 'routing' => ['rateLimits' => ['minute' => (int) $this->optionStore->get('settings.routing.rate_limits.minute', 0), 'hour' => (int) $this->optionStore->get('settings.routing.rate_limits.hour', 0), 'day' => (int) $this->optionStore->get('settings.routing.rate_limits.day', 0)], 'circuitBreaker' => ['threshold' => (int) $this->optionStore->get('settings.routing.circuit_breaker.threshold', 5), 'windowSeconds' => (int) $this->optionStore->get('settings.routing.circuit_breaker.window_seconds', 300), 'cooldownSeconds' => (int) $this->optionStore->get('settings.routing.circuit_breaker.cooldown_seconds', 300)]], 'queue' => ['retry' => ['maxRetries' => (int) $this->optionStore->get('settings.queue.retry.max_retries', 3), 'delaySeconds' => (int) $this->optionStore->get('settings.queue.retry.delay_seconds', 60), 'multiplier' => (int) $this->optionStore->get('settings.queue.retry.multiplier', 2), 'maxDelaySeconds' => (int) $this->optionStore->get('settings.queue.retry.max_delay_seconds', 900)]]], 'options' => ['deliveryModes' => $this->getDeliveryModeOptions(), 'routingStrategies' => $this->getRoutingStrategyOptions(), 'returnPathModes' => $this->getReturnPathModeOptions()]];
    }
    /**
