...
final readonly class Version202603190001CreateCoreTables implements MigrationInterface
{
    ...
    public function up(Connection $connection, TableNameResolver $tableNameResolver): void
    {
        ...
        $connection->executeStatement(sprintf('CREATE TABLE IF NOT EXISTS %s (
            ...
            ) %s', $tableNameResolver->resolve('connections'), $charsetCollation));
        $connection->executeStatement(sprintf('CREATE TABLE IF NOT EXISTS %s (
                id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
            ...
            ) %s', $tableNameResolver->resolve('queue_messages'), DatabaseTransport::NAME, $charsetCollation));
        $connection->executeStatement(sprintf('CREATE TABLE IF NOT EXISTS %s (
                id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
            ...
            ) %s', $tableNameResolver->resolve('mail_logs'), $charsetCollation));
        $connection->executeStatement(sprintf('CREATE TABLE IF NOT EXISTS %s (
                id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
            ...
            ) %s', $tableNameResolver->resolve('mail_attempts'), $charsetCollation));
        $connection->executeStatement(sprintf('CREATE TABLE IF NOT EXISTS %s (
                id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
            ...
            ) %s', $tableNameResolver->resolve('webhook_events'), $charsetCollation));
    }
