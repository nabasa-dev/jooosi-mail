 ...
 */
#[Service]
final class MailerSendWebhookAdapter extends AbstractWebhookAdapter
{
    #[Override]
    ...
    public function parse(WP_REST_Request $request, Connection $connection): array
    {
        ...
        return [[
            'mail_log_id' => $this->extractMailLogId($variables) ?? $this->extractMailLogId($payload),
            'event_type' => $this->normalizeEventType((string) ($payload['type'] ?? 'received')),
            'transport_message_id' => is_array($message) ? $this->extractFirstString($message, ['id']) : null,
            'payload' => $payload,
            'occurred_at' => $this->formatOccurredAt($payload['created_at'] ?? null),
        ]];
    }
}
