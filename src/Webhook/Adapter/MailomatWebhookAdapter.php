 ...
 */
#[Service]
final class MailomatWebhookAdapter extends AbstractWebhookAdapter
{
    #[Override]
    ...
    public function verify(WP_REST_Request $request, Connection $connection): bool
    {
        ...
        if ($algorithm === '' || $signature === '' || ! in_array($algorithm, hash_hmac_algos(), true)) {
            return false;
        }
    ...
    public function parse(WP_REST_Request $request, Connection $connection): array
    {
        ...
        return [[
            'mail_log_id' => $this->extractMailLogId($payload) ?? $this->extractMailLogId($providerPayload),
            'event_type' => $this->normalizeMailomatEvent($eventType),
            'transport_message_id' => $this->extractFirstString($payload, ['messageId', 'messageUuid']),
            'provider_event_id' => $this->extractFirstString($payload, ['id']),
            'payload' => $payload,
            'occurred_at' => $this->formatOccurredAt($payload['occurredAt'] ?? null),
        ]];
    }
