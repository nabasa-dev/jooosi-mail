 ...
 */
#[Service]
final class AhaSendWebhookAdapter extends AbstractWebhookAdapter
{
    #[Override]
    ...
    public function parse(WP_REST_Request $request, Connection $connection): array
    {
        ...
        return [[
            ...
            'event_type' => $this->normalizeAhaSendEventType((string) ($payload['type'] ?? 'received')),
            'transport_message_id' => $data === [] ? null : $this->extractFirstString($data, ['id']),
            'payload' => $payload,
            'occurred_at' => $this->formatAhaSendOccurredAt($payload['timestamp'] ?? null),
        ]];
    }
