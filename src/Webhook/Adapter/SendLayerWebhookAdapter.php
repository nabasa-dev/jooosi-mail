 ...
 */
#[Service]
final class SendLayerWebhookAdapter extends AbstractWebhookAdapter
{
    #[Override]
    ...
    public function parse(WP_REST_Request $request, Connection $connection): array
    {
        ...
        return [[
            ...
            'event_type' => $this->normalizeSendLayerEvent((string) ($eventData['Event'] ?? 'received')),
            'transport_message_id' => $this->extractFirstString($eventData, ['MessageID']),
            'payload' => $payload,
            'occurred_at' => $this->formatOccurredAt($signature['Timestamp'] ?? null),
        ]];
    }
