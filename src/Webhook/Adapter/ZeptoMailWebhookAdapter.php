 ...
 */
#[Service]
final class ZeptoMailWebhookAdapter extends AbstractWebhookAdapter
{
    #[Override]
    ...
    public function verify(WP_REST_Request $request, Connection $connection): bool
    {
        ...
        if (abs((int) round(microtime(true) * 1000) - $timestampMs) > 300000) {
            return false;
        }

        $expectedSignature = base64_encode(hash_hmac('sha256', $this->extractSignedPayload($request), $secret, true));
    ...
    public function parse(WP_REST_Request $request, Connection $connection): array
    {
        ...
        return [[
            ...
            'event_type' => $this->normalizeZeptoMailEvent((string) ($payload['event_name'] ?? 'received')),
            'transport_message_id' => $transportMessageId,
            'provider_event_id' => $providerEventId,
            'payload' => $payload,
            'occurred_at' => $this->formatOccurredAt($details['time'] ?? $details['modified_time'] ?? $emailInfo['processed_time'] ?? null),
        ]];
    }
