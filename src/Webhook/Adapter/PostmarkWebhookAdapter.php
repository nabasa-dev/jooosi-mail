 ...
 */
#[Service]
final class PostmarkWebhookAdapter extends AbstractWebhookAdapter
{
    /**
     * @var list<string>
     */
    ...
    public function parse(WP_REST_Request $request, Connection $connection): array
    {
        ...
        return [[
            ...
            'event_type' => $this->normalizeRecordType((string) ($payload['RecordType'] ?? 'received')),
            'transport_message_id' => $this->extractFirstString($payload, ['MessageID', 'OriginalMessageID', 'MessageId']),
            'payload' => $payload,
            'occurred_at' => $this->formatOccurredAt(
                $payload['ReceivedAt']
                    ?? $payload['DeliveredAt']
                    ?? $payload['BouncedAt']
                    ?? $payload['ChangedAt']
