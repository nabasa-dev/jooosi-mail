...
final class WebhookAdapterRegistry
{
    ...
    private function all(): array
    {
        ...
        foreach ($this->manifest->services as $className) {
            if (! is_subclass_of($className, WebhookAdapterInterface::class)) {
                continue;
            }
            ...
            if ($service instanceof WebhookAdapterInterface) {
                $adapters[] = $service;
            }
        }

        usort($adapters, static fn (WebhookAdapterInterface $left, WebhookAdapterInterface $right): int => $right->getPriority() <=> $left->getPriority());
