...
final readonly class WebhookEventRepository
{
    ...
    public function listRecentEventTypesByConnection(array $connectionIds, int $hours = 24, int $sampleSize = 20): array
    {
        ...
        $rows = $this->connection->createQueryBuilder()->select('connection_id', 'event_type')->from($this->tableNameResolver->resolve('webhook_events'))->where('connection_id IN (:connection_ids)')->andWhere('COALESCE(occurred_at, created_at) >= :since')->orderBy('id', 'DESC')->setMaxResults(max($sampleSize * count($connectionIds), 50))->setParameter('connection_ids', array_values($connectionIds), ArrayParameterType::INTEGER)->setParameter('since', $since)->fetchAllAssociative();
        $eventsByConnection = [];
        foreach ($rows as $row) {
