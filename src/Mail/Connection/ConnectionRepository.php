...
final readonly class ConnectionRepository
{
    ...
    public function find(int $id): ?Connection
    {
        $row = $this->connection->fetchAssociative(
            sprintf('SELECT * FROM %s WHERE id = :id LIMIT 1', $this->tableNameResolver->resolve('connections')),
            ['id' => $id],
        );
    ...
    public function findAll(): array
    {
        $rows = $this->connection->fetchAllAssociative(
            sprintf('SELECT * FROM %s ORDER BY is_enabled DESC, is_default DESC, priority ASC, id ASC', $this->tableNameResolver->resolve('connections')),
        );

        return array_map(fn (array $row): Connection => $this->hydrate($row), $rows);
    }
    ...
    public function findActive(): array
    {
        $rows = $this->connection->fetchAllAssociative(
            sprintf(
                'SELECT * FROM %s WHERE is_enabled = 1 ORDER BY is_default DESC, priority ASC, id ASC',
     ...
     * @since 0.1.0
     */
    public function findDefault(): ?Connection
    {
        $row = $this->connection->fetchAssociative(
            sprintf('SELECT * FROM %s WHERE is_enabled = 1 AND is_default = 1 ORDER BY priority ASC, id ASC LIMIT 1', $this->tableNameResolver->resolve('connections')),
        );
     ...
     * @since 0.1.0
     */
    public function save(Connection $connection): int
    {
        $payload = [
            'profile_key' => $connection->profileKey,
            'name' => $connection->name,
            'dsn' => $connection->dsn,
            'settings_json' => wp_json_encode($connection->settings),
            'secrets_json' => wp_json_encode($this->encryptSecrets($connection->secrets)),
            'is_enabled' => $connection->enabled ? 1 : 0,
            'is_default' => $connection->default ? 1 : 0,
            'priority' => $connection->priority,
            'weight' => $connection->weight,
            'webhook_enabled' => $connection->webhookEnabled ? 1 : 0,
    ...
    private function hydrate(array $row): Connection
    {
        ...
        $secrets = json_decode((string) ($row['secrets_json'] ?? '{}'), true);

        return new Connection(
            id: isset($row['id']) ? (int) $row['id'] : null,
            profileKey: (string) ($row['profile_key'] ?? ''),
            name: (string) ($row['name'] ?? ''),
            dsn: isset($row['dsn']) && (string) $row['dsn'] !== '' ? (string) $row['dsn'] : null,
            settings: is_array($settings) ? $settings : [],
            secrets: $this->decryptSecrets(is_array($secrets) ? $secrets : []),
            enabled: (bool) ($row['is_enabled'] ?? false),
            default: (bool) ($row['is_default'] ?? false),
            priority: (int) ($row['priority'] ?? 10),
            weight: (int) ($row['weight'] ?? 1),
            webhookEnabled: (bool) ($row['webhook_enabled'] ?? false),
        );
    }
