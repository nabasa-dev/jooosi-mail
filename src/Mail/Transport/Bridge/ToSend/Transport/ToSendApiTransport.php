...
final class ToSendApiTransport extends AbstractApiTransport
{
    ...
    {
        $response = $this->client->request('POST', $this->getEndpoint(), ['headers' => ['Authorization' => 'Bearer ' . $this->apiKey, 'Content-Type' => 'application/json'], 'json' => $this->getPayload($email, $envelope)]);
        try {
    ...
    {
        $payload = ['from' => $this->formatSender($email, $envelope), 'to' => $this->formatAddresses($this->getRecipients($email, $envelope)), 'subject' => (string) $email->getSubject()];
        if ($email->getCc() !== []) {
    ...
    private function getAttachments(Email $email): array
    {
        ...
        foreach ($email->getAttachments() as $dataPart) {
            ...
            $attachments[] = ['content' => base64_encode($dataPart->getBody()), 'name' => (string) ($headers->getHeaderParameter('Content-Disposition', 'filename') ?? 'attachment'), 'type' => $headers->get('Content-Type')?->getBody() ?? 'application/octet-stream'];
        }
        return $attachments;
    }
    /**
    ...
    private function formatErrorMessage(array $result): string
    {
        ...
        $messages = [...$messages, ...$this->collectErrorMessages($result['errors'] ?? [])];
    ...
    private function isAllowedCustomHeader(string $name): bool
    {
        ...
        if (in_array($normalized, self::DISALLOWED_HEADERS, \true)) {
            return \false;
        }
