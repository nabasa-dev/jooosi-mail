...
final class SmtpComApiTransport extends AbstractApiTransport
{
    ...
    {
        $response = $this->client->request('POST', 'https://' . self::HOST . '/v4/messages', ['headers' => ['Authorization' => 'Bearer ' . $this->apiKey, 'Content-Type' => 'application/json', 'Accept' => 'application/json'], 'json' => $this->buildPayload($email, $envelope)]);
        try {
            ...
            if ($statusCode !== 200) {
                ...
                throw new HttpTransportException(sprintf('Unable to send email: %s (code %d).', $message, $statusCode), $response);
            }
    ...
    {
        $payload = ['subject' => $email->getSubject(), 'to' => $this->formatRecipients($envelope->getRecipients()), 'from' => $envelope->getSender()->getAddress()];
        ...
        foreach ($email->getAttachments() as $dataPart) {
            if ($dataPart instanceof DataPart) {
                $attachments[] = ['name' => $dataPart->getFilename() ?: 'attachment', 'data' => base64_encode($dataPart->getBody()), 'type' => $dataPart->getContentType()];
            }
        }
        if ($attachments !== []) {
        ...
        foreach ($email->getHeaders()->all() as $header) {
            $name = $header->getName();
            if (in_array(strtolower($name), ['from', 'to', 'cc', 'bcc', 'subject', 'reply-to'], \true)) {
                continue;
            }
