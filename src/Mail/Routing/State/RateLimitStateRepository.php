...
final readonly class RateLimitStateRepository
{
    ...
    {
        $row = $this->connection->fetchAssociative(sprintf('SELECT * FROM %s WHERE connection_id = :connection_id AND period_key = :period_key LIMIT 1', $this->tableNameResolver->resolve('connection_rate_limits')), ['connection_id' => $connectionId, 'period_key' => $period]);
    ...
    {
        $activeLimits = array_filter($limits, static fn(int $limit, string $period): bool => $limit > 0 && isset($windowSecondsByPeriod[$period]), \ARRAY_FILTER_USE_BOTH);
        if ($activeLimits === []) {
            return \true;
        }
        $this->connection->beginTransaction();
    ...
    private function normalizeState(array $row, int $windowSeconds): array
    {
        ...
        if ($endsAt <= $now) {
            return ['count' => 0, 'started_at' => $now, 'ends_at' => $now + $windowSeconds];
        }
        return ['count' => max(0, (int) ($row['usage_count'] ?? 0)), 'started_at' => $startedAt, 'ends_at' => $endsAt];
    }
    /**
     * @since 0.1.0
     */
