...
final readonly class DeliveryPlan
{
    ...
    {
        return new self(
            mode: DeliveryMode::fromMixed($data['mode'] ?? null),
            strategy: RoutingStrategy::fromMixed($data['strategy'] ?? RoutingStrategy::WeightedRandom->value),
            priority: (int) ($data['priority'] ?? 10),
            delaySeconds: (int) ($data['delaySeconds'] ?? 0),
            preferredConnectionId: isset($data['preferredConnectionId']) ? (int) $data['preferredConnectionId'] : null,
        );
    }
