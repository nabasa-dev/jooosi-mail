...
final readonly class ConnectionCircuitBreaker
{
    ...
    public function __construct(private CircuitBreakerStateRepository $circuitBreakerStateRepository, private OptionStore $optionStore, private EventPublisherInterface $eventPublisher)
    {
    }
    /**
     * @since 0.1.0
    ...
    public function getBlacklistedUntil(Connection $connection): ?int
    {
        ...
        return isset($state['blacklisted_until']) && is_int($state['blacklisted_until']) && $state['blacklisted_until'] > time() ? $state['blacklisted_until'] : null;
    }
    /**
    ...
    public function getStatus(Connection $connection): array
    {
        ...
        $state = $connection->id !== null ? $this->circuitBreakerStateRepository->get($connection->id, $this->getWindowSeconds($connection)) : ['recent_failure_count' => 0];
        return ['enabled' => $this->isEnabled($connection) ? 1 : 0, 'threshold' => $this->getThreshold($connection), 'window_seconds' => $this->getWindowSeconds($connection), 'cooldown_seconds' => $this->getCooldownSeconds($connection), 'recent_failures' => (int) ($state['recent_failure_count'] ?? 0), 'blacklisted_until' => $blacklistedUntil];
    }
    /**
     * @since 0.1.0
     */
    ...
    {
        $threshold = $connection->settings['circuit_breaker']['threshold'] ?? $connection->settings['circuit_breaker_threshold'] ?? $this->optionStore->get('settings.routing.circuit_breaker.threshold', self::DEFAULT_THRESHOLD);
    ...
    {
        $window = $connection->settings['circuit_breaker']['window'] ?? $connection->settings['circuit_breaker_window'] ?? $this->optionStore->get('settings.routing.circuit_breaker.window_seconds', self::DEFAULT_WINDOW);
    ...
    {
        $cooldown = $connection->settings['circuit_breaker']['cooldown'] ?? $connection->settings['circuit_breaker_cooldown'] ?? $this->optionStore->get('settings.routing.circuit_breaker.cooldown_seconds', self::DEFAULT_COOLDOWN);
