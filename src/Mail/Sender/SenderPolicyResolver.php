...
final readonly class SenderPolicyResolver
{
    ...
    public function apply(MailRequest $mailRequest, Connection $connection): MailRequest
    {
        ...
        return new MailRequest(from: $from, to: $mailRequest->to, cc: $mailRequest->cc, bcc: $mailRequest->bcc, replyTo: $mailRequest->replyTo, subject: $mailRequest->subject, textBody: $mailRequest->textBody, htmlBody: $mailRequest->htmlBody, attachments: $mailRequest->attachments, headers: $mailRequest->headers, envelopeSender: $envelopeSender, source: $mailRequest->source, metadata: $mailRequest->metadata);
    }
    /**
    ...
    private function resolveFrom(MailRequest $mailRequest, array $connectionPolicy, array $globalPolicy): array
    {
        ...
        $defaultFromApplied = ($mailRequest->metadata[self::DEFAULT_FROM_APPLIED_METADATA_KEY] ?? \false) === \true;
        if (!$forceEmail && !$forceName && !$defaultFromApplied && $mailRequest->from !== []) {
            return $mailRequest->from;
        }
        $originalFrom = $mailRequest->from[0] ?? null;
        $configuredEmail = $this->extractEmail($connectionPolicy['email'] ?? null) ?? $this->extractEmail($globalPolicy['email'] ?? null);
        $configuredName = $this->extractString($connectionPolicy['name'] ?? null) ?? $this->extractString($globalPolicy['name'] ?? null);
     ...
     * @since 0.1.0
     */
    private function resolveEnvelopeSender(MailRequest $mailRequest, array $from, array $connectionPolicy, array $globalPolicy): ?MailAddress
    {
        ...
        if ($mode === self::RETURN_PATH_MODE_CUSTOM) {
            $email = $connectionMode === self::RETURN_PATH_MODE_CUSTOM ? $this->extractEmail($connectionPolicy['return_path_email'] ?? null) : $this->extractEmail($globalPolicy['return_path_email'] ?? null);
    ...
    {
        return ['email' => $this->optionStore->get('settings.mail.sender.email'), 'name' => $this->optionStore->get('settings.mail.sender.name'), 'force_email' => (bool) $this->optionStore->get('settings.mail.sender.force_email', \false), 'force_name' => (bool) $this->optionStore->get('settings.mail.sender.force_name', \false), 'return_path_mode' => $this->optionStore->get('settings.mail.sender.return_path_mode', self::RETURN_PATH_MODE_PROVIDER_DEFAULT), 'return_path_email' => $this->optionStore->get('settings.mail.sender.return_path_email')];
    }
    /**
     * @return array<string, mixed>
     *
    ...
    private function extractBool(mixed $value): bool
    {
        ...
        return in_array(strtolower(trim((string) $value)), ['1', 'true', 'yes', 'on', 'force'], \true);
    }
    /**
     * @since 0.1.0
     */
    private function normalizeConnectionReturnPathMode(mixed $value): ?string
    {
        ...
        return in_array($mode, [self::RETURN_PATH_MODE_PROVIDER_DEFAULT, self::RETURN_PATH_MODE_MATCH_FROM, self::RETURN_PATH_MODE_CUSTOM], \true) ? $mode : null;
    }
    /**
     * @since 0.1.0
     */
    private function normalizeGlobalReturnPathMode(mixed $value): string
    {
        ...
        $mode = strtolower(trim((string) $value));
        return in_array($mode, [self::RETURN_PATH_MODE_PROVIDER_DEFAULT, self::RETURN_PATH_MODE_MATCH_FROM, self::RETURN_PATH_MODE_CUSTOM], \true) ? $mode : self::RETURN_PATH_MODE_PROVIDER_DEFAULT;
    }
    /**
     * @since 0.1.0
     */
