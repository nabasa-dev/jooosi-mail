...
#[MailProfile(
    key: 'pepipost',
    label: 'Pepipost',
    description: 'Send mail through Pepipost using custom API or SMTP transports.',
    website: 'https://netcorecloud.com/email/',
    docsUrl: 'https://emaildocs.netcorecloud.com/reference/send-mail-api-1',
...
final class PepipostProfile extends AbstractMailProfile
{
    ...
    {
        return [
            'scheme' => ['label' => 'Transport scheme', 'type' => 'choice', 'required' => false, 'default' => 'pepipost+api', 'choices' => $this->getSupportedSchemes()],
            'api_key' => ['label' => 'Pepipost API key', 'type' => 'password', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', 'pepipost+api')], 'required_when' => [$this->conditionIn('scheme', 'pepipost+api')]],
            'username' => ['label' => 'Pepipost SMTP username', 'type' => 'text', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', ['pepipost+smtp', 'pepipost+smtps'])], 'required_when' => [$this->conditionIn('scheme', ['pepipost+smtp', 'pepipost+smtps'])]],
            'password' => ['label' => 'Pepipost SMTP password', 'type' => 'password', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', ['pepipost+smtp', 'pepipost+smtps'])], 'required_when' => [$this->conditionIn('scheme', ['pepipost+smtp', 'pepipost+smtps'])]],
            'region' => ['label' => 'Pepipost region', 'type' => 'choice', 'required' => false, 'choices' => ['eu'], 'visible_when' => [$this->conditionIn('scheme', 'pepipost+api')]],
        ];
    }
