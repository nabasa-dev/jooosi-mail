...
#[MailProfile(
    key: 'smtp2go',
    label: 'SMTP2GO',
    description: 'Send mail through SMTP2GO using custom API or SMTP transports.',
    website: 'https://www.smtp2go.com',
    docsUrl: 'https://developers.smtp2go.com/docs/send-an-email',
...
final class Smtp2goProfile extends AbstractMailProfile
{
    ...
    {
        return [
            'scheme' => ['label' => 'Transport scheme', 'type' => 'choice', 'required' => false, 'default' => 'smtp2go+api', 'choices' => $this->getSupportedSchemes()],
            'api_key' => ['label' => 'SMTP2GO API key', 'type' => 'password', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', 'smtp2go+api')], 'required_when' => [$this->conditionIn('scheme', 'smtp2go+api')]],
            'username' => ['label' => 'SMTP2GO SMTP username', 'type' => 'text', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', ['smtp2go+smtp', 'smtp2go+smtps'])], 'required_when' => [$this->conditionIn('scheme', ['smtp2go+smtp', 'smtp2go+smtps'])]],
            'password' => ['label' => 'SMTP2GO SMTP password', 'type' => 'password', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', ['smtp2go+smtp', 'smtp2go+smtps'])], 'required_when' => [$this->conditionIn('scheme', ['smtp2go+smtp', 'smtp2go+smtps'])]],
            'region' => ['label' => 'SMTP2GO region', 'type' => 'choice', 'required' => false, 'default' => 'global', 'choices' => ['global', 'us', 'eu', 'eu2', 'au']],
        ];
    }
