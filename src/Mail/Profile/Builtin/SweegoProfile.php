...
#[MailProfile(
    key: 'sweego',
    label: 'Sweego',
    description: 'Send mail through Sweego using the Symfony bridge API or SMTP transport.',
    website: 'https://www.sweego.io',
    docsUrl: 'https://learn.sweego.io/',
...
final class SweegoProfile extends AbstractMailProfile
{
    ...
    {
        return [
            'scheme' => ['label' => 'Transport scheme', 'type' => 'choice', 'required' => false, 'default' => 'sweego+api', 'choices' => $this->getSupportedSchemes()],
            'api_key' => ['label' => 'Sweego API key', 'type' => 'password', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', 'sweego+api')], 'required_when' => [$this->conditionIn('scheme', 'sweego+api')]],
            'host' => ['label' => 'Sweego SMTP host', 'type' => 'text', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', 'sweego+smtp')], 'required_when' => [$this->conditionIn('scheme', 'sweego+smtp')]],
            'port' => ['label' => 'Sweego SMTP port', 'type' => 'number', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', 'sweego+smtp')], 'required_when' => [$this->conditionIn('scheme', 'sweego+smtp')]],
            'username' => ['label' => 'Sweego SMTP login', 'type' => 'text', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', 'sweego+smtp')], 'required_when' => [$this->conditionIn('scheme', 'sweego+smtp')]],
            'password' => ['label' => 'Sweego SMTP password', 'type' => 'password', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', 'sweego+smtp')], 'required_when' => [$this->conditionIn('scheme', 'sweego+smtp')]],
        ];
    }
