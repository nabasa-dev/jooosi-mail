...
#[MailProfile(
    key: 'gmail',
    label: 'Gmail',
    description: 'Send mail through Gmail using either app-password SMTP or a Google Workspace service account via the Gmail API.',
...
final class GmailProfile extends AbstractMailProfile
{
    ...
    {
        return [
            'scheme' => ['label' => 'Transport scheme', 'type' => 'choice', 'required' => false, 'default' => 'gmail+smtp', 'choices' => $this->getSupportedSchemes()],
            'username' => ['label' => 'Gmail address', 'type' => 'text', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', 'gmail+smtp')], 'required_when' => [$this->conditionIn('scheme', 'gmail+smtp')]],
            'password' => ['label' => 'Gmail app password', 'type' => 'password', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', 'gmail+smtp')], 'required_when' => [$this->conditionIn('scheme', 'gmail+smtp')]],
            'service_account_email' => ['label' => 'Google service account email', 'type' => 'text', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', 'gmail+api')], 'required_when' => [$this->conditionIn('scheme', 'gmail+api')]],
            'private_key' => ['label' => 'Google service account private key', 'type' => 'password', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', 'gmail+api')], 'required_when' => [$this->conditionIn('scheme', 'gmail+api')]],
            'user_email' => ['label' => 'Delegated Gmail sender address', 'type' => 'text', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', 'gmail+api')], 'required_when' => [$this->conditionIn('scheme', 'gmail+api')]],
        ];
    }
