...
#[MailProfile(
    ...
    description: 'Send mail through Microsoft Graph using the Symfony bridge API transport.',
...
final class MicrosoftGraphProfile extends AbstractMailProfile
{
    ...
    {
        return [
            'scheme' => ['label' => 'Transport scheme', 'type' => 'choice', 'required' => false, 'default' => 'microsoftgraph+api', 'choices' => $this->getSupportedSchemes()],
            'client_app_id' => ['label' => 'Microsoft Graph client app ID', 'type' => 'text', 'required' => true],
            'client_app_secret' => ['label' => 'Microsoft Graph client app secret', 'type' => 'password', 'required' => true],
            'tenant_id' => ['label' => 'Microsoft Graph tenant ID', 'type' => 'text', 'required' => true],
            'graph_endpoint' => ['label' => 'Microsoft Graph endpoint host', 'type' => 'text', 'required' => false],
            'auth_endpoint' => [
                'label' => 'Microsoft auth endpoint host',
                'type' => 'text',
                'required' => false,
                'required_message' => 'Configuration field "auth_endpoint" is required for profile "microsoftgraph" when using a custom graph endpoint.',
                'visible_when' => [
                    $this->conditionNotIn('graph_endpoint', ['', 'default', 'graph.microsoft.com']),
                ],
                'required_when' => [
                    $this->conditionNotIn('graph_endpoint', ['', 'default', 'graph.microsoft.com']),
                ],
            ],
            'no_save' => ['label' => 'Do not save to Sent Items', 'type' => 'choice', 'required' => false, 'default' => 'false', 'choices' => ['false', 'true']],
        ];
    }
    ...
    public function buildDsn(Connection $connection): ?string
    {
        ...
        $query = $this->buildQueryString([
            'tenantId' => $tenantId,
            'authEndpoint' => $this->normalizeAuthEndpoint($defaults['auth_endpoint'] ?? null, $graphEndpoint),
            'noSave' => $this->normalizeNoSave($defaults['no_save'] ?? null),
        ]);
