...
#[MailProfile(
    key: 'mailersend',
    label: 'MailerSend',
    description: 'Send mail through MailerSend using the Symfony bridge API or SMTP transport.',
...
final class MailerSendProfile extends AbstractMailProfile
{
    ...
    {
        return [
            'scheme' => ['label' => 'Transport scheme', 'type' => 'choice', 'required' => false, 'default' => 'mailersend+api', 'choices' => $this->getSupportedSchemes()],
            'api_key' => ['label' => 'MailerSend API key', 'type' => 'password', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', 'mailersend+api')], 'required_when' => [$this->conditionIn('scheme', 'mailersend+api')]],
            'username' => ['label' => 'MailerSend SMTP username', 'type' => 'text', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', 'mailersend+smtp')], 'required_when' => [$this->conditionIn('scheme', 'mailersend+smtp')]],
            'password' => ['label' => 'MailerSend SMTP password', 'type' => 'password', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', 'mailersend+smtp')], 'required_when' => [$this->conditionIn('scheme', 'mailersend+smtp')]],
        ];
    }
