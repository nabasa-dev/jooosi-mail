...
#[MailProfile(
    key: 'brevo',
    label: 'Brevo',
    description: 'Send mail through Brevo using the Symfony bridge API or SMTP transport.',
...
final class BrevoProfile extends AbstractMailProfile
{
    ...
    {
        return [
            'scheme' => ['label' => 'Transport scheme', 'type' => 'choice', 'required' => false, 'default' => 'brevo+api', 'choices' => $this->getSupportedSchemes()],
            'api_key' => ['label' => 'Brevo API key', 'type' => 'password', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', 'brevo+api')], 'required_when' => [$this->conditionIn('scheme', 'brevo+api')]],
            'username' => ['label' => 'Brevo SMTP username', 'type' => 'text', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', 'brevo+smtp')], 'required_when' => [$this->conditionIn('scheme', 'brevo+smtp')]],
            'password' => ['label' => 'Brevo SMTP password', 'type' => 'password', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', 'brevo+smtp')], 'required_when' => [$this->conditionIn('scheme', 'brevo+smtp')]],
        ];
    }
