...
#[MailProfile(
    key: 'zohomail',
    label: 'Zoho Mail',
    description: 'Send mail through Zoho Mail using the custom SMTP transport.',
...
final class ZohoMailProfile extends AbstractMailProfile
{
    ...
    {
        return [
            'scheme' => ['label' => 'Transport scheme', 'type' => 'choice', 'required' => false, 'default' => 'zohomail+smtp', 'choices' => $this->getSupportedSchemes()],
            'username' => ['label' => 'Zoho Mail username', 'type' => 'text', 'required' => true],
            'password' => ['label' => 'Zoho Mail password', 'type' => 'password', 'required' => true],
            'account_type' => ['label' => 'Zoho account type', 'type' => 'choice', 'required' => false, 'default' => 'personal', 'choices' => ['personal', 'business', 'pro']],
        ];
    }
    ...
    public function buildDsn(Connection $connection): ?string
    {
        ...
        if (! in_array($scheme, $this->getSupportedSchemes(), true) || $username === null || $username === '' || $password === null || $password === '') {
            return null;
        }

        $query = $this->buildQueryString([
            'account_type' => $this->normalizeAccountType($defaults['account_type'] ?? null),
        ]);
