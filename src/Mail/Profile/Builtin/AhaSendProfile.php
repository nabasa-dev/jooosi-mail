...
#[MailProfile(
    key: 'ahasend',
    label: 'AhaSend',
    description: 'Send mail through AhaSend using the Symfony bridge API or SMTP transport.',
    website: 'https://ahasend.com',
    docsUrl: 'https://ahasend.com/docs',
...
final class AhaSendProfile extends AbstractMailProfile
{
    ...
    {
        return [
            'scheme' => ['label' => 'Transport scheme', 'type' => 'choice', 'required' => false, 'default' => 'ahasend+api', 'choices' => $this->getSupportedSchemes()],
            'api_key' => ['label' => 'AhaSend API key', 'type' => 'password', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', 'ahasend+api')], 'required_when' => [$this->conditionIn('scheme', 'ahasend+api')]],
            'username' => ['label' => 'AhaSend SMTP username', 'type' => 'text', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', 'ahasend+smtp')], 'required_when' => [$this->conditionIn('scheme', 'ahasend+smtp')]],
            'password' => ['label' => 'AhaSend SMTP password', 'type' => 'password', 'required' => false, 'visible_when' => [$this->conditionIn('scheme', 'ahasend+smtp')], 'required_when' => [$this->conditionIn('scheme', 'ahasend+smtp')]],
        ];
    }
