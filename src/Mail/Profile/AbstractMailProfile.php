 ...
 * @since 0.1.0
 */
abstract class AbstractMailProfile implements MailProfileInterface
{
    /**
     * @since 0.1.0
     */
    ...
    protected function conditionIn(string $field, array|string $values): array
    {
        return [
            'field' => $field,
            'operator' => 'in',
            'values' => $this->normalizeConditionValues($values),
        ];
    }
    ...
    private function getStoredConfigurationValue(?Connection $existingConnection, string $name, bool $secret): mixed
    {
        ...
        $profileData = $secret
            ? $this->getProfileSecrets($existingConnection)
            : $this->getProfileSettings($existingConnection);
